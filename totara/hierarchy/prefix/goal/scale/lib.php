...
function goal_scale_is_used($scaleid) {
    ...
    $personal_use = $DB->record_exists('goal_personal', array('scaleid' => $scaleid));
...
function goal_scale_display_table($scales, $editingon=0) {
    ...
    $can_delete = has_capability('totara/hierarchy:deletegoal', $sitecontext);
    ...
    if ($scales) {
        ...
        foreach ($scales as $scale) {
            ...
            if ($editingon) {
                ...
                if ($can_delete) {
                    if ($scale_used) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletegoalscaleinuse', 'totara_hierarchy'), 'totara_core',
                            array('class' => 'iconsmall', 'title' => get_string('error:nodeletegoalscaleinuse', 'totara_hierarchy')));
                    } else if ($scale_assigned) {
                        $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletegoalscaleassigned', 'totara_hierarchy'), 'totara_core',
                            array('class' => 'iconsmall', 'title' => get_string('error:nodeletegoalscaleassigned', 'totara_hierarchy')));
                    } else {
