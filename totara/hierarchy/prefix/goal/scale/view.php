...
$can_edit = has_capability('totara/hierarchy:updategoal', $sitecontext);
...
if ($can_edit) {
    ...
    require_capability('totara/hierarchy:viewgoal', $sitecontext);
} else {
...
$str_changeto = get_string('changeto', 'totara_hierarchy');
$str_set = get_string('set', 'totara_hierarchy');
...
if ($can_edit) {
    ...
    if ((!empty($moveup) or !empty($movedown))) {
        ...
        if (goal_scale_is_used($scale->id)) {
            $returnurl = new moodle_url('/totara/hierarchy/prefix/goal/scale/view.php', array('id' => $scale->id, 'prefix' => 'goal'));
    ...
    foreach ($values as $value) {
        ...
        if ($can_edit) {
            ...
            if (!$scale_used) {
                ...
                if ($value->id == $scale->defaultid) {
                    $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletegoalscalevaluedefault', 'totara_hierarchy'), 'totara_core',
                        array('class' => 'iconsmall'));
                // Prevent deleting last proficient value.
                } else if ($value->id == $onlyprof) {
                    $buttons[] = $OUTPUT->pix_icon('t/delete_grey', get_string('error:nodeletegoalscalevalueonlyprof', 'totara_hierarchy'), 'totara_core',
                        array('class' => 'iconsmall'));
                } else {
            ...
            if ($count > 1 && !$scale_used) {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/totara/hierarchy/prefix/goal/scale/view.php',
                    array('id' => $scale->id, 'moveup' => $value->id, 'prefix' => 'goal')), new pix_icon('t/up', $str_moveup), null,
                    array('class' => 'iconsmall', 'title' => $str_moveup));
            } else {
            ...
            if ($count < $numvalues && !$scale_used) {
                $buttons[] = $OUTPUT->action_icon(new moodle_url('/totara/hierarchy/prefix/goal/scale/view.php',
                    array('id' => $scale->id, 'movedown' => $value->id, 'prefix' => 'goal')), new pix_icon('t/down', $str_movedown), null,
                    array('class' => 'iconsmall', 'title' => $str_movedown));
            } else {
                $buttons[] = $OUTPUT->spacer(array('height' => 11, 'width' => 11));
            }
        } else {
