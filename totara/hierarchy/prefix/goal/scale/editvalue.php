...
if ($id == 0) {
    ...
    require_capability('totara/hierarchy:creategoal', $sitecontext);
    ...
    // Editing scale value.
    require_capability('totara/hierarchy:updategoal', $sitecontext);
...
if ($valueform->is_cancelled()) {
    ...
    if ($valuenew->id == 0) {
        ...
        if ($valuenew->id = $DB->insert_record('goal_scale_values', $valuenew)) {
            // Log.
            add_to_log(SITEID, 'goal', 'added scale value', "prefix/goal/scale/view.php?id={$valuenew->scaleid}&amp;prefix=goal");
            $notification->text = 'scalevalueadded';
            $notification->url = "$CFG->wwwroot/totara/hierarchy/prefix/goal/scale/view.php?id={$valuenew->scaleid}&amp;prefix=goal";
        ...
        if ($DB->update_record('goal_scale_values', $valuenew)) {
            // Log.
            add_to_log(SITEID, 'goal', 'update scale value',
                "prefix/goal/scale/view.php?id={$valuenew->scaleid}&amp;prefix=goal");
            $notification->text = 'scalevalueupdated';
            $notification->url = "$CFG->wwwroot/totara/hierarchy/prefix/goal/scale/view.php"
                . "?id={$valuenew->scaleid}&amp;prefix=goal";
            $notification->params = array('class' => 'notifysuccess');
        } else {
