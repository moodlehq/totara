...
// Indicates whether currently assigned items, not in $add list, should be deleted.
...
if (empty($returnurl)) {
    switch ($assigntype) {
        case GOAL_ASSIGNMENT_INDIVIDUAL:
            $returnurl = "/totara/hierarchy/prefix/goal/mygoals.php?id={$assignto}";
            break;
...
if ($add) {
    ...
    foreach ($items as $item) {
        ...
        if ($assigntype == GOAL_ASSIGNMENT_INDIVIDUAL) {
            ...
            if (!$DB->record_exists('goal_record', array('goalid' => $item, 'userid' => $assignto))) {
                $DB->insert_record('goal_record', $scale_default);
            }
        } else {
    ...
    if ($nojs) {
        ...
        switch ($assigntype) {
            ...
            case GOAL_ASSIGNMENT_ORGANISATION:
                $renderer = $PAGE->get_renderer('totara_hierarchy');
                $addgoalurl = new moodle_url('/totara/hierarchy/prefix/goal/assign/find.php', array('assignto' => $assignto, 'assigntype' => GOAL_ASSIGNMENT_POSITION));
