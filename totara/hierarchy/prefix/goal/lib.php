...
class goal extends hierarchy {
    ...
    public function hierarchy_page_setup($page = '', $item=null) {
        ...
        $selected_position = json_encode(dialog_display_currently_selected(
            get_string('selected', 'totara_hierarchy'), 'position'));
        $selected_organisation = json_encode(dialog_display_currently_selected(
            get_string("currentlyselected", "totara_hierarchy"), "organisation"));
        $args = array('args'=>'{"userid":'.$USER->id.','.
                      '"can_edit": true,'.
                      '"sesskey":"'.sesskey().'",'.
                      '"dialog_display_position":'.$selected_position.','.
                      '"dialog_display_organisation":'.$selected_organisation.'}');
        $PAGE->requires->strings_for_js(array('chooseposition', 'choosemanager', 'chooseorganisation', 'assigngroup'), 'totara_hierarchy');
    ...
    public function display_extra_view_info($item) {
        ...
        if ($can_assign) {
            echo html_writer::select($options, 'groupselector', null, null, array('class' => 'group_selector', 'itemid' => $item->id));
        }
    }
    ...
    public function get_item_data($item, $cols = null) {
        ...
        if (!empty($item->targetdate)) {
            $format_date = userdate($item->targetdate, get_string('strftimedatefullshort', 'langconfig'), $CFG->timezone, false);
            $data[] = array(
    ...
    public function update_user_assignments($goalid, $type, $assignment) {
        ...
        foreach ($new as $user) {
            if ($index = array_search($user, $current_index, true)) {
                ...
                if (!$DB->record_exists('goal_record', array('goalid' => $goalid, 'userid' => $user))) {
                    $DB->insert_record('goal_record', $default_scale);
                }
            }
        }
     ...
     * @param boolean $transfer Whether or not to completely delete records or just move them
     */
    public static function delete_user_assignments($type, $assignmentid) {
        global $DB;
        ...
        // Find all of the records that we want to delete, then delete them.
        $assignmentparams = array('assigntype' => $type, 'assignmentid' => $assignmentid);
        $userassignments = $DB->get_records('goal_user_assignment', $assignmentparams);
        $DB->delete_records('goal_user_assignment', $assignmentparams);

        // For each goal/user user_assginment deleted, see if there are any other user_assignments.
        ...
        foreach ($userassignments as $userassignment) {
            ...
            if (!$DB->record_exists('goal_user_assignment', $recordparams)) {
                $DB->delete_records('goal_record', $recordparams);
            }
        }
    }
    ...
    public function create_user_assignments($type, $assignment, $includechildren = null) {
        ...
        $users = $DB->get_fieldset_select($typeinfo->members_table, 'userid', "{$typeinfo->members_field} = ?", array($assignment->$field));
        ...
        if (!empty($includechildren)) {
            ...
            foreach ($children as $child) {
                $assigned = $DB->get_fieldset_select($typeinfo->members_table, 'userid', "{$typeinfo->members_field} = ?", array($child->id));
        ...
        foreach ($users as $user) {
            ...
            $DB->insert_record('goal_user_assignment', $user_assignment);
            if (!$DB->record_exists('goal_record', array('goalid' => $assignment->goalid, 'userid' => $user))) {
                $DB->insert_record('goal_record', $default_scale);
            }
        }
        ...
        foreach ($childusers as $childuser) {
            ...
            $DB->insert_record('goal_user_assignment', $user_assignment);
            if (!$DB->record_exists('goal_record', array('goalid' => $assignment->goalid, 'userid' => $childuser))) {
                $DB->insert_record('goal_record', $default_scale);
            }
        }
    }
     ...
     */
    public static function get_user_assignments($userid, $canedit = null) {
        global $DB, $OUTPUT;
        ...
        foreach ($assignments as $assignment) {
            ...
            $scalevalueid = $DB->get_field('goal_record', 'scalevalueid',
                    array('userid' => $assignment->userid, 'goalid' => $assignment->goalid));

            if ($assignment->assigntype == GOAL_ASSIGNMENT_INDIVIDUAL && $canedit) {
                // Set up the edit and delete icons.
                $del_params = array('goalid' => $assignment->goalid, 'assigntype' => $assignment->assigntype, 'modid' => $assignment->userid);
            ...
            if (!empty($assignment_info[$goalid])) {
                ...
                $assignment_info[$goalid]->via .= html_writer::empty_tag('br') . html_writer::empty_tag('hr') .
                    self::get_assignment_string('company', $assignment) . $del_button;
            } else {
                // Create a new object for this role.
                $assignment_info[$goalid] = new stdClass();

                // Get the goal out of the database.
                ...
                $assignment_info[$goalid]->via = self::get_assignment_string('company', $assignment) . $del_button;
            }
        }
    ...
    /**
     * Get all the personal level goals that a user is assigned to.
     *
     * @param   int     $userid the id of the user to get the records of
     * @return  array   A collection of goal records matching the userid
     */
    public static function get_user_personalgoals($userid) {
        global $DB;

        $goals = $DB->get_records('goal_personal', array('userid' => $userid));
     ...
     *
     * @param string $goaltype     The type of goal (personal/company)
     * @param int $assignment      The database record of a goal assignment
     * @return string              Formatted for the my goals table
     */
    public static function get_assignment_string($goaltype, $assignment) {
        global $DB;
        ...
        if ($goaltype == 'personal') {
            ...
            switch ($assignment->assigntype) {
                ...
                case GOAL_ASSIGNMENT_AUDIENCE:
                    ...
                    $assignment_string = get_string('goalassignmentaudience', 'totara_hierarchy', format_string($audience->name));
                    break;
                case GOAL_ASSIGNMENT_POSITION:
                    // Position: {$posname}.
                    ...
                    $assignment_string = get_string('goalassignmentposition', 'totara_hierarchy', format_string($pos->fullname));
                    break;
                case GOAL_ASSIGNMENT_ORGANISATION:
                    ...
                    $assignment_string = get_string('goalassignmentorganisation', 'totara_hierarchy', format_string($org->fullname));
                    break;
                default:
                    // Unrecognised assignment type.
     ...
     * @param int $goalid       The id of the associated goal, used to retrieve the name
    ...
    public static function goal_assignment_type_info($assigntype, $goalid = null, $modid = null) {
        ...
        switch ($assigntype) {
            ...
            case GOAL_ASSIGNMENT_ADMIN:
                ...
                // Get the goal to retrieve a few variables.
                $goal = !empty($goalid) ? $DB->get_record('goal_personal', array('id' => $goalid)) : null;
    ...
    public static function tab_visible($userid = null) {
        global $DB, $USER;
        ...
        $has_personal = $DB->record_exists('goal_personal', array('userid' => $userid));
        $add_personal = has_capability('totara/hierarchy:manageownpersonalgoal', $usercontext);
        $personal = $view_personal && ($has_personal || $add_personal);
        ...
        $company = $view_company && ($has_company || $add_company);
