...
class goal_edit_personal_form extends moodleform {
    ...
    public function set_data($data) {
        ...
        if (!empty($data->targetdate)) {
            ...
            $data->targetdateselector = userdate($data->targetdate, get_string('strftimedatefullshort', 'langconfig'), $CFG->timezone, false);
        }

        if (!empty($data->description)) {
            // Same again for the description.
            $data->descriptionformat = FORMAT_HTML;
            $data = file_prepare_standard_editor($data, 'description', $options, $options['context'], 'totara_hierarchy', 'goal', $data->id);
        }
    ...
    public function validation($itemnew, $files) {
        ...
        // Check user exists.
        if (!$DB->record_exists('user', array('id' => $itemnew->user))) {
            $errors['user'] = get_string('userdoesnotexist', "totara_core");
        }

        // Check scale exists.
        if (!empty($itemnew->scale) && !$DB->record_exists('goal_scale', array('id' => $itemnew->scale))) {
        ...
        if (!empty($itemnew->targetdateselector)) {
            $targetdate = $itemnew->targetdateselector;
            if (!empty($targetdate)) {
                if ($date = totara_date_parse_from_format($dateparseformat, $targetdate)) {
                    ...
                    $errors['targetdateselector'] = get_string('error:invaliddateformat', 'totara_hierarchy', get_string('datepickerplaceholder', 'totara_core'));
                }
            }
        }
