...
$goalid = required_param('goal', PARAM_INT);
...
$ret_params = array('id' => $userid);
$ret_url = new moodle_url("/totara/hierarchy/prefix/goal/mygoals.php", $ret_params);

if (!$goal = $DB->get_record('goal_personal', array('id' => $goalid))) {
...
if ($delete) {
    ...
    if ($delete != md5($goal->timemodified)) {
    ...
    // Do the deletion.
    if ($DB->delete_records('goal_personal', array('id' => $goalid))) {
        $success = get_string('deletedpersonalgoal', 'totara_hierarchy', format_string($goal->name));
        add_to_log(SITEID, 'goal', 'delete personal goal', $ret_url, format_string($goal->name) . " (ID $goal->id)");
        totara_set_notification($success, $ret_url, array('class' => 'notifysuccess'));
    } else {
        // Failure.
        $error = get_string('error:deletepersonalgoal', 'totara_hierarchy', format_string($goal->name));
...
$PAGE->navbar->add(get_string('mygoals', 'totara_hierarchy'),
    new moodle_url('/totara/hierarchy/item/prefix/goal/mygoals.php', array('id' => $userid)));
$PAGE->navbar->add(format_string($goal->name),
    new moodle_url('/totara/hierarchy/prefix/goal/item/view.php', array('goal' => $goalid)));
$PAGE->navbar->add(get_string('deletegoal', 'totara_hierarchy'));

echo $OUTPUT->header();

$str_param = new stdClass();
$str_param->goalname = $goal->name;
...
$del_params = array('goal' => $goalid, 'user' => $userid, 'del' => md5($goal->timemodified), 'sesskey' => $USER->sesskey);
