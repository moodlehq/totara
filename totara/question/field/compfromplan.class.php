...
class question_compfromplan extends review {
    public function get_info() {
        return array('group' => question::GROUP_REVIEW, 'title' => get_string('questiontypecompfromplan', 'totara_question'));
    }

    public function __construct($definition, $prefix, $subjectid = 0, $answerfield = '', $answerid = 0) {
        $this->reviewtitle = get_string('reviewyourlpcompetencies', 'totara_question');
        $this->buttonlabel = get_string('choosecompreview', 'totara_question');
    ...
    public function edit_form(MoodleQuickForm $form) {
        ...
        $PAGE->requires->strings_for_js(array('choosecompreview', 'removeconfirm'), 'totara_question');
        ...
        if (!$plan) {
            $form->addElement('static', 'noplans', '', get_string('nocompplans', 'totara_question'));
            return;
        }
        ...
        $jsmodule = array(
            'name' => 'totara_plan_competency',
            'fullpath' => '/totara/question/field/compfromplan.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.totara_plan_competency.init', $args, false, $jsmodule);
        ...
        $form->addElement('static', $form_prefix . '_review_competencies', $this->name, $html);
        $this->render_without_label($form, $form_prefix.'_review_competencies');
    }


    public function get_display_query($itemid) {
        $sql = 'SELECT questdata.*, c.fullname FROM {'.$this->prefix.'_review_data} questdata
                JOIN {dp_plan_competency_assign} pca ON questdata.itemid = pca.id
                JOIN {comp} c ON c.id = pca.competencyid
                WHERE questdata.id = ?';
        $params = array($itemid);
    ...
    public function get_items_query() {
        $sql = 'SELECT questdata.*, c.fullname FROM {'.$this->prefix.'_review_data} questdata
                JOIN {dp_plan_competency_assign} pca ON
                    questdata.itemid = pca.id
                JOIN {comp} c ON
                    c.id = pca.competencyid
                WHERE
