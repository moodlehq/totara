...
class question_evidencefromplan extends review {
    public function get_info() {
        return array('group' => question::GROUP_REVIEW, 'title' => get_string('questiontypeevidencefromplan', 'totara_question'));
    }

    public function __construct($definition, $prefix, $subjectid = 0, $answerfield = '', $answerid = 0) {
        $this->reviewtitle = get_string('reviewyourlpevidence', 'totara_question');
        $this->buttonlabel = get_string('chooseevidencereview', 'totara_question');
    ...
    public function edit_form(MoodleQuickForm $form) {
        ...
        $PAGE->requires->string_for_js('chooseevidencereview', 'totara_question');
        ...
        if (!$plan) {
            $form->addElement('static', 'noplans', '', get_string('noevidenceplans', 'totara_question'));
            return;
        }
        ...
        $jsmodule = array(
            'name' => 'totara_plan_evidence',
            'fullpath' => '/totara/question/field/evidencefromplan.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.totara_plan_evidence.init', $args, false, $jsmodule);
        ...
        $form->addElement('static', $form_prefix . '_review_evidence', $this->name, $html);
        $this->render_without_label($form, $form_prefix.'_review_evidence');
    }
    ...
    public function get_items_query() {
        $sql = 'SELECT questdata.*, pe.name as fullname FROM {'.$this->prefix.'_review_data} questdata
                JOIN {dp_plan_evidence} pe ON
                    questdata.itemid = pe.id
                WHERE
