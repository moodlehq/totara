...
class question_progfromplan extends review {
    public function get_info() {
        return array('group' => question::GROUP_REVIEW, 'title' => get_string('questiontypeprogfromplan', 'totara_question'));
    }

    public function __construct($definition, $prefix, $subjectid = 0, $answerfield = '', $answerid = 0) {
        $this->reviewtitle = get_string('reviewyourlpprograms', 'totara_question');
        $this->buttonlabel = get_string('chooseprogramreview', 'totara_question');
    ...
    public function edit_form(MoodleQuickForm $form) {
        ...
        $PAGE->requires->strings_for_js(array('chooseprogramreview', 'removeconfirm'), 'totara_question');
        ...
        if (!$plan) {
            $form->addElement('static', 'noplans', '', get_string('noprogramplans', 'totara_question'));
            return;
        }
        ...
        $jsmodule = array(
            'name' => 'totara_plan_program',
            'fullpath' => '/totara/question/field/progfromplan.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.totara_plan_program.init', $args, false, $jsmodule);
        ...
        $form->addElement('static', $form_prefix . '_review_programs', $this->name, $html);
        $this->render_without_label($form, $form_prefix.'_review_programs');
    }


    public function get_display_query($itemid) {
        $sql = 'SELECT p.fullname FROM {'.$this->prefix.'_review_data} questdata
            JOIN {dp_plan_program_assign} pa ON questdata.itemid = pa.id
            JOIN {prog} p ON p.id = pa.programid
            WHERE questdata.id = ?';
        $params = array($itemid);
    ...
    public function get_items_query() {
        $sql = 'SELECT questdata.*, p.fullname FROM {'.$this->prefix.'_review_data} questdata
                JOIN {dp_plan_program_assign} pa ON
                    questdata.itemid = pa.id
                JOIN {prog} p ON
                    pa.programid = p.id
                WHERE
