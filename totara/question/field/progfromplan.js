 ...
 */
M.totara_plan_program = M.totara_plan_program || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.totara_plan_prgoram.init()-> jQuery dependency required for this module to function.');
        }

        var url = M.cfg.wwwroot + '/totara/appraisal/ajax/';
        var saveurl = url + 'reviewplanprogram.php?id='+this.config.question_id+'&answerid='+this.config.answerid+'&formprefix='+this.config.formprefix+'&subjectid='+this.config.subjectid+'&update=';
        ...
        handler._update = function(response) {
            ...
            M.totara_plan_program.totara_appraisal_question_update(response);
        };
        ...
        totaraDialogs['appraisalprogfromplan'] = new totaraDialog(
            'assignprograms',
            'id_' + this.config.formprefix + '_choosereviewitem',
            {
                buttons: buttonsObj,
                title: '<h2>' + M.util.get_string('chooseprogramreview', 'totara_question') + '</h2>'
            },
            url+'planprogram.php?id='+this.config.question_id+'&answerid='+this.config.answerid+'&planid='+this.config.plan_id+'&subjectid='+this.config.subjectid,
            handler
        );

        M.totara_plan_program.addActions();
    },
    ...
    addActions: function() {
        $('.' + this.config.formprefix + 'appraisal_review').find('a.action-icon.delete').on('click', function(){
            M.totara_plan_program.modalDelete($(this).attr('href'), $(this).attr('data-reviewitemid'), $(this).closest('tr'));
            return false;
        });
    }
};
