 ...
 */
M.totara_plan_course = M.totara_plan_course || {
    ...
    init: function(Y, args){
        ...
        if (typeof $ === 'undefined') {
            throw new Error('M.totara_plan_course.init()-> jQuery dependency required for this module to function.');
        }

        var url = M.cfg.wwwroot + '/totara/appraisal/ajax/';
        var saveurl = url + 'reviewplancourse.php?id='+this.config.question_id+'&answerid='+this.config.answerid+'&formprefix='+this.config.formprefix+'&subjectid='+this.config.subjectid+'&update=';
        ...
        handler._update = function(response) {
            ...
            M.totara_plan_course.totara_appraisal_question_update(response);
        };
        ...
        totaraDialogs['appraisalplancourse'] = new totaraDialog(
            'assigncourses',
            'id_' + this.config.formprefix + '_choosereviewitem',
            {
                buttons: buttonsObj,
                title: '<h2>' + M.util.get_string('choosecoursereview', 'totara_question') + '</h2>'
            },
            url+'plancourse.php?id='+this.config.question_id+'&answerid='+this.config.answerid+'&planid='+this.config.plan_id+'&subjectid='+this.config.subjectid,
            handler
        );

        M.totara_plan_course.addActions();
    },
