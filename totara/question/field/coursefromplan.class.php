...
class question_coursefromplan extends review {
    public function get_info() {
        return array('group' => question::GROUP_REVIEW, 'title' => get_string('questiontypecoursefromplan', 'totara_question'));
    }

    public function __construct($definition, $prefix, $subjectid = 0, $answerfield = '', $answerid = 0) {
        $this->reviewtitle = get_string('reviewyourlpcourses', 'totara_question');
        $this->buttonlabel = get_string('choosecoursereview', 'totara_question');
    ...
    public function edit_form(MoodleQuickForm $form) {
        ...
        $PAGE->requires->string_for_js('choosecoursereview', 'totara_question');
        ...
        if (!$plan) {
            $form->addElement('static', 'noplans', '', get_string('nocourseplans', 'totara_question'));
            return;
        }
        ...
        $jsmodule = array(
            'name' => 'totara_plan_course',
            'fullpath' => '/totara/question/field/coursefromplan.js',
            'requires' => array('json'));
        $PAGE->requires->js_init_call('M.totara_plan_course.init', $args, false, $jsmodule);
        ...
        $form->addElement('static', $form_prefix . '_review_courses', $this->name, $html);
        $this->render_without_label($form, $form_prefix.'_review_courses');
    }


    // Currently only used in to_html delete if we don't end up using to_html
    // for the review questions
    public function get_display_query($itemid) {
        $sql = 'SELECT c.fullname FROM {'.$this->prefix.'_review_data} questdata
            JOIN {dp_plan_course_assign} pca ON questdata.itemid = pca.id
    ...
    public function get_items_query() {
        $sql = 'SELECT questdata.*, c.fullname FROM {'.$this->prefix.'_review_data} questdata
            JOIN {dp_plan_course_assign} pca ON
