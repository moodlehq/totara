...
abstract class question_base_form extends moodleform {
    ...
    public static function get_header(question_base $element) {
        if ($element->id > 0) {
            $header = ($element->has_editable()) ? 'questioneditheader' : 'questioninfoeditheader';
        } else {
            $header = ($element->has_editable()) ? 'questionaddheader' : 'questioninfoaddheader';
        }
        $info = $element->get_info();
        return get_string($header, 'totara_question', $info['title']);
    }
}
...
class question_choose_element_form extends moodleform {
     ...
     */
    public function definition($excludegroups = array()) {
        $mform =& $this->_form;
        $group = array();
        $options = array();
        $questionman = new question($this->prefix);
        $elements = $questionman->get_registered_elements();
        $elem_group = '';
        foreach ($elements as $key => $elem) {
            ...
            switch ($elem['group']) {
                case question::GROUP_QUESTION:
                    $grouplabel = get_string('groupquestion', 'totara_question');
                    break;
                case question::GROUP_REVIEW:
            ...
            $options[$grouplabel][$key] = $elem['title'];
        }
