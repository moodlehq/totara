...
switch ($action) {
    ...
    case 'edit':
        if ($mnewform->is_submitted()) {
            if ($newelement = $mnewform->get_data()) {
            } else {
        ...
        if ($question->id < 1) {
            $question->attach_element($questionman->get_element(required_param('datatype', PARAM_ACTION)));
        }
...
switch ($action) {
    ...
    default:
        ...
        $mnewform->display();
        echo $output->list_questions($questions);
