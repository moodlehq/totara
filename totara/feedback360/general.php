...
if ($fromform = $mform->get_data()) {
    ...
    $feedback360->set($todb)->save();

    add_to_log(SITEID, 'feedback360', 'update feedback360', 'general.php?id='.$id, 'General Settings: feedback360 ID=' . $id);
    if ($id > 0) {
        totara_set_notification(get_string('feedback360updated', 'totara_feedback360'), $returnurl, array('class' => 'notifysuccess'));
    } else {
        $stageurl = new moodle_url('/totara/feedback360/general.php', array('id' => $feedback360->id));
        totara_set_notification(get_string('feedback360updated', 'totara_feedback360'), $stageurl, array('class' => 'notifysuccess'));
    }
}
