...
if (!empty($confirmation)) {
    ...
    if ($confirmation == $valid) {
        ...
        if (!empty($userform->timedue)) {
            $duedate = userdate($userform->timedue, get_string('strftimedatefullshort', 'langconfig'));
        ...
        foreach ($resp_assignments as $resp_assignment) {
            ...
            if (!empty($resp_assignment->feedback360emailassignmentid)) {
                ...
                feedback360::email_external_address($email_assignment->email, $requester, $emailsubject, $emailstr, $emailstralt);
            } else {
                ...
                $eventdata->subject = $stringmanager->get_string('reminderemailsubject', 'totara_feedback360', $remvars, $userto->lang);
                $eventdata->fullmessage = $stringmanager->get_string('reminderemailbody', 'totara_feedback360', $remvars, $userto->lang);
...
foreach ($resp_assignments as $resp_assignment) {
    ...
    if (!empty($resp_assignment->feedback360emailassignmentid)) {
        $external .= format_string($DB->get_field('feedback360_email_assignment', 'email', array('id' => $resp_assignment->feedback360emailassignmentid)));
        $external .= $spacer;
    } else {
...
echo $OUTPUT->confirm($strremind, $rem_url, $ret_url);
