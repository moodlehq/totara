...
function local_display_progressbar($percent, $size='medium', $showlabel=false, $tooltip='DEFAULTTOOLTIP') {
    ...
    switch ($size) {
        case 'large' :
            $bar = $OUTPUT->pix_url('t/progressbar-large');
            $bar_background = $OUTPUT->pix_url('t/progressbar_back-large');
            $pixelvalue = ($percent / 100) * 121;
            $pixeloffset = round($pixelvalue - 120);
            break;
        case 'medium' :
        default :
            $bar = $OUTPUT->pix_url('t/progressbar-medium');
            $bar_background = $OUTPUT->pix_url('t/progressbar_back-medium');
    ...
    $out .= '<img src="'.$bar.'" alt="' . $percent . '%" class="totaraprogressbar"
        ...
        title="'.$tooltip.'" />';
    if ($showlabel) {
        $out .= ' ' . get_string('xpercentcomplete', 'totara_core', $percent) . "<br />\n";
    }
