 ...
 */
class totara_assign_core_grouptype_pos extends totara_assign_core_grouptype {
     ...
     * @access public
     * @param $sourceid int the source table id
     * @param $groupids array All group ids associated with the assignment -
     *                  For a cohort just the base assignment but Hierarchy assignments may also have a series of child groups
     ...
     * @return array of ids
     */
    public function get_groupassignment_ids($sourceid, $includechildren=0) {
        global $DB;
        $sql = "SELECT id FROM {{$this->grouptype}} source
                WHERE id = :sourceid1";
        $params = array('sourceid1' => $sourceid);
        if ($includechildren == 1) {
            $sql .= " OR (path = :path1 OR " . $DB->sql_like('source.path', ':path2') . ")";
            $params['path1'] = "/{$sourceid}";
            $params['path2'] = "%/{$sourceid}/%";
        }
