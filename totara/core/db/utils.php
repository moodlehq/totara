...
function totara_get_text_broken_rules() {
    ...
    $customfield = $DB->sql_concat("'customfield'", 'usinfi.id', $DB->sql_like_escape("'_'"), "'%'");
    ...
    $sqlcustomrules = "cr.ruletype = :usercustomfield
                       AND cr.name IN (SELECT DISTINCT(rules.name) FROM {cohort_rules} as rules
                                       INNER JOIN {user_info_field} as usinfi ON rules.name LIKE $customfield
                                       WHERE usinfi.datatype = :texttype)";
...
function totara_cohort_remap_rules($rulestofix) {
    ...
    foreach ($rulestofix as $ruleparam) {
        ...
        $todb->ruleid = $ruleparam->ruleid;
        $todb->name = $ruleparam->name;
...
function totara_preupgrade($totarainfo) {
    ...
    set_config('existingtotaraversion', $totarainfo->existingtotaraversion);
