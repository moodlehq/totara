...
class importcertification_testcase extends advanced_testcase {
    ...
    public function test_import() {
        ...
        $this->assertEquals(CERT_HISTORY_IMPORT_CERTIFICATIONS, $DB->count_records('prog'), 'Record count mismatch in program table');
        ...
        $sql = "INSERT INTO {certif}
                ...
                FROM {prog}";
        $DB->execute($sql);
        $this->assertEquals(CERT_HISTORY_IMPORT_CERTIFICATIONS, $DB->count_records('certif'), 'Record count mismatch for certif');
        ...
        $this->assertEquals(CERT_HISTORY_IMPORT_USERS+2, $DB->count_records('user'), 'Record count mismatch for users'); // Guest + Admin + generated users.
        ...
        $this->assertEquals(CERT_HISTORY_IMPORT_CSV_ROWS, $DB->count_records($importtablename), 'Record count mismatch in the import table ' . $importtablename);
        $this->assertEquals(0, $DB->count_records('dp_plan_evidence'), 'There should be no evidence records');
        $this->assertEquals(CERT_HISTORY_IMPORT_CSV_ROWS, $DB->count_records('certif_completion'), 'Record count mismatch in the certif_completion table');
        $this->assertEquals(CERT_HISTORY_IMPORT_CSV_ROWS, $DB->count_records('prog_completion'), 'Record count mismatch in the prog_completion table');
        $this->assertEquals(CERT_HISTORY_IMPORT_CSV_ROWS, $DB->count_records('prog_user_assignment'), 'Record count mismatch in the prog_user_assignment table');
    }
}
