...
class importcourse_testcase extends advanced_testcase {
    ...
    public function test_import() {
        ...
        $this->assertEquals(COURSE_HISTORY_IMPORT_USERS+1, $DB->count_records('course'), 'Record count mismatch for courses');
        ...
        $this->assertEquals(COURSE_HISTORY_IMPORT_COURSES+2, $DB->count_records('user'), 'Record count mismatch for users');

        // Manual enrol should be set
        $this->assertEquals(COURSE_HISTORY_IMPORT_COURSES, $DB->count_records('enrol', array('enrol'=>'manual')), 'Manual enrol is not set for all courses');
        ...
        $this->assertEquals(COURSE_HISTORY_IMPORT_CSV_ROWS, $DB->count_records($importtablename), 'Record count mismatch in the import table ' . $importtablename);
        $this->assertEquals(0, $DB->count_records('dp_plan_evidence'), 'There should be no evidence records');
        $this->assertEquals(COURSE_HISTORY_IMPORT_CSV_ROWS, $DB->count_records('course_completions'), 'Record count mismatch in the course_completions table');
        $this->assertEquals(COURSE_HISTORY_IMPORT_CSV_ROWS, $DB->count_records('user_enrolments'), 'Record count mismatch in the user_enrolments table');
    }
}
