...
switch ($action) {
    case 'getroles' :
        echo html_writer::tag('div', $output->display_roles($stages[$id]->roles), array('class' => 'cananswer'));
        $allviewers = $stage->get_roles_involved(appraisal::ACCESS_CANVIEWOTHER);
        $viewers = array_diff_key($allviewers, $stages[$id]->roles);
        echo html_writer::tag('div', $output->display_roles($viewers), array('class' => 'canview'));
        break;
    default:
