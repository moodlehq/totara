...
// Calculate the previous question's permissions.
$previousquestion = null;
$previous_quest_perms = '';
if ($id == 0) {
    // Get all answerable questions.
    $allquestions = appraisal_question::fetch_appraisal($stage->appraisalid, null, appraisal::ACCESS_CANANSWER);
    if ($allquestions) {
        ...
        while (($pointer < count($allquestions)) &&
              ($pointed->stagetimedue > $stage->timedue) ||
               ((($pointed->stagetimedue == $stage->timedue) && ($pointed->appraisalstageid > $page->appraisalstageid)) ||
                (($pointed->appraisalstageid == $page->appraisalstageid) && ($pointed->pagesortorder > $page->sortorder)))) {
            $pointer++;
            if ($pointer < count($allquestions)) {
                $pointed = $allquestions[$pointer];
            }
        }
        // If pointer is not beyond the end of the array then it is pointing to the correct question.
        if ($pointer < count($allquestions)) {
            $previousquestion = new appraisal_question($pointed->id);
        }
    }
...
switch ($action) {
    case 'duplicate':
        ...
        $questions = appraisal_question::get_list($page->id);
        if (is_ajax_request($_SERVER)) {
            echo 'success';
            return;
        }
    ...
    case 'edit':
        if ($id == 0) {
            ...
            if ($fromaddelemform = $mnewform->get_data()) {
                $question->attach_element($questionman->get_element($fromaddelemform));
            } else {
                $datatype = required_param('datatype', PARAM_ACTION);
                $question->attach_element($questionman->get_element($datatype));
            }
        }
