...
class appraisal_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        if (appraisal::is_draft($appraisal->id)) {
            ...
            $mform->addElement('static', 'description_static', get_string('description'), $appraisal->description_editor['text']);
        }
...
class appraisal_answer_form extends moodleform {
    ...
    public function definition_after_data() {
        ...
        $userassignment = $this->_customdata['userassignment'];
        ...
        $stageiscomplete = $stage->is_completed($userassignment, $roleassignment);
        $pageislocked = $readonly || $page->is_locked($userassignment, $roleassignment, $preview);
        ...
        $questions = appraisal_question::fetch_page_role($page->id, $roleassignment->appraisalrole);

        // Set required property.
        $rolecodestrings = appraisal::get_roles();
        foreach ($questions as $quest) {
            $appraisal_question = new appraisal_question($quest->id, $roleassignment);
            $isviewonlyquestion = true;
            $elem = $appraisal_question->get_element();

            if (($quest->rights & appraisal::ACCESS_CANANSWER) == appraisal::ACCESS_CANANSWER) {
                $isviewonlyquestion = false;
                if (!$showformfields) {
                    $elem->set_viewonly(true);
                } else if (($quest->rights & appraisal::ACCESS_MUSTANSWER) == appraisal::ACCESS_MUSTANSWER) {
                    $elem->set_required(true);
                }

                $viewerroles = $appraisal_question->get_roles_involved(appraisal::ACCESS_CANVIEWOTHER);
                ...
                if (!empty($viewerroles)) {
                    ...
                    foreach ($viewerroles as $viewerrole) {
                        if ($viewerrole != $roleassignment->appraisalrole) {
                            $viewers[] = get_string($rolecodestrings[$viewerrole], 'totara_appraisal');
                        }
                    }

                    if (!empty($viewers) && !$readonly) {
                        // Add "visible to" item
                        $viewersstring = '<small class="visibleto">' . get_string('visibleto', 'totara_appraisal') .
                            '<br>' . implode(', ', $viewers) . '</small>';
                        $elem->viewersstring = $viewersstring;
                    }
                }
            }

            if (($quest->rights & appraisal::ACCESS_CANVIEWOTHER) == appraisal::ACCESS_CANVIEWOTHER) {
                // Get who can answer (get assignments).
                $questroles = $appraisal_question->roles;
                ...
                foreach ($questroles as $eachrole => $rights) {
                    if (($rights & appraisal::ACCESS_CANANSWER) == appraisal::ACCESS_CANANSWER) {
                        ...
                        $questioninfo = appraisal_question::get_manager($subject->id, $otherassignments[$eachrole]->id);
                        ...
                        $elem->add_question_role_info($eachrole, $questioninfo);
                    }
                }
            }

            if ((($quest->rights & appraisal::ACCESS_CANANSWER) != appraisal::ACCESS_CANANSWER) && !$isviewonlyquestion) {
    ...
    public function validation($data, $files) {
        ...
        $questions = appraisal_question::fetch_page_role($data['pageid'], $data['role']);
        foreach ($questions as $quest) {
            $appraisal_question = new appraisal_question($quest->id, $roleassignment);
            $newerrors = $appraisal_question->get_element()->edit_validate($data);
            if (!empty($newerrors)) {
...
class appraisal_stage_edit_form extends moodleform {
    ...
    public function definition() {
        ...
        $submittitle = get_string('addstage', 'totara_appraisal');
        if ($stage->id > 0) {
            $submittitle = get_string('savechanges', 'moodle');
        }
...
class appraisal_quest_edit_form extends question_base_form {
    ...
    public function definition() {
        ...
        $stagename = $this->_customdata['stage']->name;
        $pagename = $this->_customdata['page']->name;
...
class appraisal_message_form extends moodleform {
    ...
    public function definition() {
        ...
        foreach ($stages as $stage) {
            $eventslist[$stage->id] = get_string('eventstage', 'totara_appraisal', $stage->name);
        }
...
class appraisal_print_stages_form extends moodleform {
    public function definition()  {
        ...
        foreach ($stages as $stagedata) {
            $stageselect[] = $elem = $mform->createElement('advcheckbox', $stagedata->id, '', $stagedata->name);
