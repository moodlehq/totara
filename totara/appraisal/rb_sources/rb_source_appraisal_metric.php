...
class rb_source_appraisal_metric extends rb_source_appraisal {
        ...
        foreach ($allquestionrecords as $allquestionrecord) {
            ...
            foreach ($roles as $role) {
                if (isset($questionrecords[$role][$allquestionrecord->id])) {
                    ...
                    if ($role == $lastincludedrole) {
                        // This is the last role to display, so include the maximum column.
                        $results = array_merge($results,
                                $this->get_columns_for_question($question, $columnoption, $hidden));
                    } else {
                        // There are more roles to show, so exclude the maximum column.
                        $results = array_merge($results,
                                $this->get_columns_for_question($question, $columnoption, $hidden, false));
                    }
                }
            }
        }
    ...
    private function get_columns_for_question($question, $columnoption, $hidden, $includemax = true) {
        global $DB, $FILEPICKER_OPTIONS;

        $rolename = get_string($columnoption->type, 'totara_appraisal');

        $results = array();
        switch ($question->get_element()->get_type()) {
            case 'text':
                $newcolumn =
                    $this->make_question_column(
                        $columnoption->type,
                        $columnoption->value . "_" . $question->id,
                        $question->name . ' (' . $rolename . ')',
                ...
                if ($includemax) {
                    ...
                    $newcolumn2 =
                        $this->make_question_column(
                            ...
                            get_string('ratingcustommaximum', 'rb_source_appraisal_metric', $question->name),
                            '(' . $max . ')',
                            $columnoption,
                            $hidden
                        );
                ...
                if ($includemax) {
                    ...
                    $newcolumn2 =
                        $this->make_question_column(
                            $columnoption->type,
                            $columnoption->value . '_' . $question->id . '_maximum',
                            get_string('ratingnumericmaximum', 'rb_source_appraisal_metric', $question->name),
                            '(' . $max . ')',
                            $columnoption,
                            $hidden
                        );
            ...
            case 'multichoicesingle':
                ...
                $joinname = 'scalevalue' . $question->id;
                $this->joinlist[] =
                    new rb_join(
                        $joinname,
                        'LEFT',
            ...
            case '--todo--':
            case '-- all review question types --':
            case '--dontinclude--':
            case 'fixedtext':
            case 'fixedimage':
            case 'userinfo':
