...
class totara_appraisal_renderer extends plugin_renderer_base {
    ...
    public function report_active_table($appraisals = array()) {
        ...
        $tableheader = array(get_string('name', 'totara_appraisal'),
                             ...
                             get_string('metrics', 'totara_appraisal'));
        ...
        foreach ($appraisals as $appraisal) {
            $row = array();
            $statusurl = new moodle_url('/totara/appraisal/statusreport.php',
                    array('appraisalid' => $appraisal->id, 'clearfilters' => 1));
            $row[] = html_writer::link($statusurl, format_string($appraisal->name));
            ...
            if ($appraisal->usersoverdue > 0) {
                $statusurl->param('filterstatus', 'statusoverdue');
                $row[] = html_writer::link($statusurl, $appraisal->usersoverdue);
            } else {
                $row[] = '0';
            }
            if ($appraisal->usersontarget > 0) {
                $statusurl->param('filterstatus', 'statusontarget');
                $row[] = html_writer::link($statusurl, $appraisal->usersontarget);
            } else {
                $row[] = '0';
            }
            if ($appraisal->userscomplete > 0) {
                $statusurl->param('filterstatus', 'statuscomplete');
                $row[] = html_writer::link($statusurl, $appraisal->userscomplete);
            } else {
                $row[] = '0';
            }
            $metricurl = new moodle_url('/totara/appraisal/metricreport.php',
                    array('appraisalid' => $appraisal->id, 'clearfilters' => 1));
            $row[] = html_writer::link($metricurl, get_string('viewreport', 'totara_appraisal'));
            $data[] = $row;
        }
        $appraisalstable->data = $data;
    ...
    public function report_inactive_table($appraisals = array()) {
        ...
        foreach ($appraisals as $appraisal) {
            ...
            if ($appraisal->userstotal - $appraisal->userscomplete > 0) {
                $statusurl->param('filterstatus', 'statusincomplete');
                $row[] = html_writer::link($statusurl, $appraisal->userstotal - $appraisal->userscomplete);
            } else {
                $row[] = '0';
            }
            $metricurl = new moodle_url('/totara/appraisal/metricreport.php',
    ...
    public function display_user_appraisals($appraisals, $role) {
        ...
        foreach ($appraisals as $appraisal) {
            ...
            if ($role != appraisal::ROLE_LEARNER) {
                $user = $DB->get_record('user', array('id' => $appraisal->userid), '*', MUST_EXIST);
                $row[] = fullname($user);
            }
    ...
    public function appraisal_stages_table($stages = array(), $userid = 0, $stageid = 0) {
        ...
        foreach ($stages as $stage) {
            ...
            $delete = $this->output->action_icon($deleteurl, new pix_icon('/t/delete', $strdelete, 'moodle'), null, array('class' => 'action-icon delete'));

            $row = new html_table_row();
            if ($stageid == $stage->id) {
                $row->attributes['class'] = 'selected';
            }
            $row->cells[] = $this->output->action_link($contenturl, format_string($stage->name), null, array('data-id' => $stage->id, 'data-type' => 'stage', 'class' => 'appraisal-stagelink'));
            if ($stage->timedue) {
            ...
            foreach ($pages as $page) {
                ...
                if ($page->id != $first->id) {
                    $posupurl = new moodle_url('/totara/appraisal/ajax/page.php', array('action' => 'posup',
                        'id' => $page->id));
                    $posuplink = $this->output->action_icon($posupurl, new pix_icon('/t/up', $strup, 'moodle'), null, array('class' => 'action-icon js-hide'));
                } else {
                    $attrs['class'] .= ' first';
                }
                if ($page->id != $last->id) {
                    $posdownurl = new moodle_url('/totara/appraisal/ajax/page.php', array('action' => 'posdown',
                            'id' => $page->id));
                    $posdownlink = $this->output->action_icon($posdownurl,new pix_icon('/t/down', $strdown, 'moodle'), null, array('class' => 'action-icon js-hide'));
                } else {
                    $attrs['class'] .= ' last';
                }
                ...
                $editlink = $this->output->action_icon($editurl, new pix_icon('/t/edit', $stredit, 'moodle'), null, array('class' => 'action-icon edit'));
                $deletelink = $this->output->action_icon($deleteurl, new pix_icon('/t/delete', $strdelete, 'moodle'), null, array('class' => 'action-icon delete'));
    ...
    public function list_questions($quests, appraisal_stage $stage) {
        ...
        if ($quests) {
            ...
            foreach ($quests as $quest) {
                ...
                $strquest = html_writer::tag('span', $questname, array('class' => 'appraisal-quest-list-name'));

                $actions = '';
                if (appraisal::is_draft($stage->appraisalid)) {
                    $actions = html_writer::tag('span',
                            $posuplink . $posdownlink . $dragdrop . $editlink . $duplicatelink . $deletelink,
    ...
    public function confirm_appraisal_activation($appraisal, $errors) {
        ...
        if (!empty($errors)) {
            ...
            $buttons[] = $this->output->single_button(new moodle_url('/totara/appraisal/stage.php',
                    array('appraisalid' => $appraisal->id)), get_string('backtoappraisal', 'totara_appraisal', $appraisal->name),
                    'get');
     ...
     * @return string HTML
     */
    public function confirm_appraisal_close($appraisal) {
        global $DB;

        $incompleteusers = $DB->count_records('appraisal_user_assignment',
                array('appraisalid' => $appraisal->id, 'timecompleted' => null));
    ...
    public function confirm_delete_appraisal($appraisalid, array $content) {
        ...
        if (count($content['stages']) > 0) {
            ...
            foreach ($content['stages'] as $stage) {
                $stages .= html_writer::tag('li', $stage->name);
            }
    ...
    public function display_appraisal_header($appraisal, $userassignment, $preview) {
        $out = html_writer::tag('h3', $appraisal->name);
     ...
     * @return string HTML
     */
    public function display_appraisal_actions($appraisal, $userassignment, $urlparams, $preview) {
        $out = '';
        $buttons = '';
    ...
    public function display_stage($appraisal, $stage, $userassignment, $roleassignment, $actions, $preview) {
        global $DB, $USER, $TEXTAREA_OPTIONS;

        // Initialise some variables.
        $allroles = appraisal::get_roles();
        $user = $DB->get_record('user', array('id' => $userassignment->userid), '*', MUST_EXIST);
        $stageinprogress = ($stage->id == $userassignment->activestageid) && empty($userassignment->timecompleted);

        // Title block (on left).
        $title = html_writer::tag('h3', $stage->name);
        ...
        foreach ($rolescompletion as $rolecompletion) {
            ...
            if (isset($rolecompletion->timecompleted)) {
                ...
                $icon = $this->output->pix_icon('i/completion-manual-n', get_string('completed', 'totara_appraisal'));
            }
            // Text.
            if ($rolecompletion->appraisalrole == $roleassignment->appraisalrole) {
                ...
                $rolecomplete = get_string('rolecompleteuser', 'totara_appraisal', fullname($user));
    ...
    public function display_stage_actions_for_stages($appraisal, $stage, $userassignment, $roleassignment, $urlparams, $preview) {
        ...
        if ($preview) {
            ...
            $action .= $this->output->pix_icon('tick2', get_string('completed', 'totara_appraisal'), 'totara_appraisal', array('class' => 'stage-complete'));
        } else if ($stage->id == $userassignment->activestageid) {
            if ($appraisal->status == appraisal::STATUS_CLOSED ||
                    $stage->is_completed($userassignment, $roleassignment)) {
     ...
     * @return string HTML
     */
    public function display_stages($appraisal, $stages, $userassignment, $roleassignment, $preview = false) {
        $out = '';
        ...
        $out .= $this->display_appraisal_actions($appraisal, $userassignment, $urlparams, $preview);
     ...
     * @param appraisal_page $page The currently selected page
     * @param object $userassignment
     * @param object $roleassignment
     * @param boolean $preview
     * @return string HTML
     */
    public function display_pages($pages, $page, $userassignment, $roleassignment, $preview = false, $includewrapper = false) {
        $out = '';
        ...
        $showsaveprogress = !$preview && isset($page) && ($roleassignment->activepageid == $page->id) &&
                ...
                !$page->is_completed($userassignment, $roleassignment);
        ...
        foreach ($pages as $tabpage) {
            ...
            if ($pageiscomplete && !$preview && ($tabpage->appraisalstageid == $userassignment->activestageid) &&
                ...
                $pageiscomplete = $tabpage->is_completed($userassignment, $roleassignment);
            }
            if (!$pageiscomplete && !$preview) {
                ...
                $icon = $this->output->pix_icon('i/completion-manual-n', get_string('incomplete', 'totara_appraisal'));
                    ...
                    !$tabpage->is_locked($userassignment, $roleassignment)) {
            ...
            $rows[] = new tabobject($tabpage->id, new moodle_url('/totara/appraisal/myappraisal.php',
                    ...
                    $icon . ' ' . $tabpage->name, $tabpage->name);
        }
        $tabs[] = $rows;
        if ($includewrapper) {
     ...
     * @return string HTML
     */
    public function display_viewing_appraisal_header($subjectid, $customlangtext = '', $role = 0) {
        global $CFG, $DB;
     ...
     *
     * @param array $items
     * @param type $form_prefix
     * @return string HTML
     */
    public function display_review_items($items, $form_prefix, $otherroles = array()) {
        global $CFG, $DB;
        ...
        foreach ($items as $item) {
            $deleteurl = new moodle_url('/totara/appraisal/ajax/removeitem.php', array('id' => $item->appraisalquestfieldid,
                'answerid' =>$item->appraisalroleassignmentid, 'itemid' => $item->itemid, 'reviewid' => $item->id,
                'sesskey' => sesskey()));
            $deleteicon = $this->output->action_icon($deleteurl, new pix_icon('t/delete', get_string('delete')), null, array('class' => 'action-icon delete', 'data-reviewitemid' => $item->id));
            $cells = array();
            $cells[] = $item->fullname;

            $answers = '';

            // If a user can answer a question and entry form element
            if (isset($item->cananswer)) {
                $formelement = new MoodleQuickForm_textarea($form_prefix . '_reviewitem_' . $item->id, '', $text_area_options);
                $formelement->setValue($item->content);
                $answers .= $formelement->toHtml();
            }

            // Add a frozen form element for all other roles that can view
            foreach ($otherroles as $role) {
                $answers .= html_writer::empty_tag('br');
                $answers .= $role->userimage;
                $answers .= $role->label . ':';

                // TODO: Find a better way of doing this, too hacky (and db calls in render are bad)
                if ($role_review_answer = $DB->get_record('appraisal_review_data', array('itemid' => $item->itemid, 'appraisalroleassignmentid' => $role->answerid, 'appraisalquestfieldid' => $item->appraisalquestfieldid, 'itemid' => $item->itemid))) {
                    $answers .= html_writer::empty_tag('br') . $role_review_answer->content;
                }
            }
            $cells[] = $answers;
            $cells[] = $deleteicon;
            $rows[] = new html_table_row($cells);
            $inputs .= html_writer::empty_tag('input', array('type' => 'hidden', 'name' => $form_prefix . "reviewitems[$item->id]", 'value' => $item->id));
        }
        ...
        $out = html_writer::tag('div', html_writer::table($table), array('class' => $form_prefix . "appraisal_review"));
        $out .= $inputs;
        return $out;
    }
    ...
    public function display_preview_header($appraisal, $role, $urlparams) {
        ...
        // The heading.
        $a = new stdClass();
        $a->appraisalname = $appraisal->name;
