...
$roleassignment = $appraisal->get_role_assignment($subjectid, $USER->id, $role, $preview);
...
$userassignment = $appraisal->get_user_assignment($subjectid, $preview);
...
if ($action == 'pages') {
    ...
    if ($preview) {
        ...
        $userassignment->activestageid = required_param('stageid', PARAM_INT);
    }
    ...
    if (!empty($visiblepages)) {
        ...
        $form = new appraisal_answer_form(null, array('appraisal' => $appraisal, 'page' => $page,
            'userassignment' => $userassignment, 'roleassignment' => $roleassignment, 'otherassignments' => $otherassignments,
        ...
        if (!$preview) {
            ...
            if (!$formissubmitted) {
                ...
                $form->set_data($appraisal->get_answers($roleassignment));
                ...
                if (($answers->submitaction == 'saveprogress') && ($roleassignment->activepageid == $pageid)) {
                    ...
                    $appraisal->save_answers($answers, $userassignment, $roleassignment, false);
                    ...
                    $pageislocked = $page->is_locked($userassignment, $roleassignment);
                    ...
                    if (!$formisvalid) {
                        ...
                        // Save valid data.
                        if ($appraisal->save_answers($answers, $userassignment, $roleassignment)) {
            ...
            foreach ($otherassignments as $otherassignment) {
                $form->set_data($appraisal->get_answers($otherassignment));
            }
        }
    }
}
...
$PAGE->requires->js_init_call('M.totara_appraisal_myappraisal.init',  array('args' => json_encode(array(
    'appraisalid' => $appraisal->id, 'role' => $role, 'subjectid' => $subjectid))), false, $jsmodule);
...
if ($preview) {
    ...
    echo $renderer->display_viewing_appraisal_header($subjectid);
}
...
// Output page content.
if ($action == 'stages') {
    echo $renderer->display_stages($appraisal, $stages, $userassignment, $roleassignment, $preview);
} else {
    if (isset($form)) {
        echo $renderer->display_pages($visiblepages, $page, $userassignment, $roleassignment, $preview, true);
        echo $OUTPUT->container_start('verticaltabtree-content');
        $form->display();
        echo $OUTPUT->container_end();
        echo $OUTPUT->container_end(); // TODO is this supposed to be twice? If so then please document.
    } else {
        echo $renderer->display_pages($visiblepages, $page, $userassignment, $roleassignment, $preview);
    }
