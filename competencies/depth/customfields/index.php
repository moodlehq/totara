...
switch ($action) {
    case 'movecategory':
        ...
        if (confirm_sesskey()) {
            customfield_move_category($id, $dir, $tableprefix);
        }   
        redirect($redirect);
        break;
   case 'movefield':
    ...
    case 'deletecategory':
        ...
        if (data_submitted() and $confirm and confirm_sesskey()) {
            customfield_delete_category($id, $depth, $tableprefix);
            redirect($redirect);
        }   
    ...
    case 'editfield':
        ...
        customfield_edit_field($id, $datatype, $depth, $redirect, $tableprefix);
        die;
        break;
    case 'editcategory':
        $id = optional_param('id', 0, PARAM_INT);

        customfield_edit_category($id, $depth, $redirect, $tableprefix);
        die;
        break;
    default:
}
...
if ($depth) {
    ...
    $competencydepth = get_record_select('competency_depth', "id='$depth'");
    $sql = "SELECT cf.fullname FROM {$CFG->prefix}competency_depth cd, {$CFG->prefix}competency_framework cf WHERE cf.id = cd.frameworkid and cd.id='$depth'";
    $competencyframeworkfullname = get_field_sql($sql);
    $competency = get_record_select('competency', "id='$depth'");
    $categories = get_records_select('competency_depth_info_category', "depthid='$depth'", 'sortorder ASC');
    ...
    foreach($categories as $category) {
        ...
        if ($fields = get_records_select('competency_depth_info_field', "categoryid=$category->id", 'sortorder ASC')) {
            ...
            print_heading(format_string($category->name).' '.customfield_category_icons($category, $categorycount, $fieldcount, $depth));

            foreach ($fields as $field) {
                $table->data[] = array($field->fullname, customfield_edit_icons($field, $fieldcount, $depth));
            }   
        } else {
            print_heading(format_string($category->name).' '.customfield_category_icons($category, $categorycount, 0, $depth));
        }
    ...
    popup_form('index.php?id=0&amp;action=editfield&amp;depth='.$depth.'&amp;datatype=', $options, 'newfieldform','','choose','','',false,'self',get_string('createnewcustomfield', 'customfields'));

    // Create a new category link
    $options = array('action'=>'editcategory', 'depth'=>$depth);
