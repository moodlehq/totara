...
/**
 * Strings for component 'group', language 'ja', branch 'MOODLE_22_STABLE'
 *
 * @package   group
...
$string['newpicture_help'] = 'あなたのコンピュータから、このサーバにイメージをアップロードすることができます。アップロードしたイメージは様々な場所であなたを表すために使用されます。
この理由から、顔のクローズアップイメージが一番良いのですが、あなたが好きなイメージを使用することもできます。
画像は、JPGまたはPNG形式を使用してください (ファイル名は通常.jpgまたは.pngで終わります)。
あなたは、多くの方法で画像ファイルを取得することができます。次に例をあげます。
1.デジタルカメラを使用する。ほとんどの場合、写真は正しい形式であなたのコンピュータの中に保存されます。
2.プリント写真をスキャナでイメージファイルに取り込むこともできます。
3.あなたが絵を描けるのでしたら、ペイントプログラムで絵を描くこともできます。

あなたが選択するどのような方法でも、使用したいイメージがJPGまたはPNGフォーマットであることを確認してください。
イメージをアップロードするには、このページの「参照」ボタンをクリックして、ハードディスクよりイメージを選択してください。
注意: アップロードできませんので、ファイルサイズが最大サイズを超えないようにしてください。
そして「プロファイルを更新する」ボタンをクリックしてください。 イメージが取り込まれて、100×100ピクセルにリサイズされます。
プロファイルページに戻ってもイメージが表示されなかったり、変更されない場合、あなたのブラウザの「更新」ボタンをクリックしてください。';
