...
/**
 * Strings for component 'role', language 'ja', branch 'MOODLE_22_STABLE'
 *
 * @package   role
...
$string['my:configsyspages'] = 'マイMoodleページのシステムテンプレートを設定する';
...
$string['overridepermissions_help'] = 'オーバーライドは、コンテクストのロールをオーバーライドするためデザインされたパーミッションです。オーバーライドでは、必要に応じてあなたのパーミッションを「調整」することができます。
...
$string['roles'] = 'ロール';
$string['roles_help'] = 'ロールはコンテクストでユーザに割り当てることのできる、システム全体に定義されたパーミッション群です。
...
ロールには、**名称**を定義してください。多言語のロールを設定したい場合、下記のような多言語シンタックスを使用することができます。
Teacher
Profesor
多言語シンタックスを使用する場合、あなたのMoodleの「フィルタ設定」が有効にされていることを確認してください。

**省略名**は、あなたのロールを他のMoodleプラグインが参照するために必要です (例 ファイルからユーザをアップロードまたはユーザ登録プラグイン経由でユーザ登録を設定する場合等)。

**説明**では、ロールに対して誰でも共通の理解ができるよう、あなたの言葉で端的にロールを説明してください。
';
