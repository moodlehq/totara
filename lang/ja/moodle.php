...
/**
 * Strings for component 'moodle', language 'ja', branch 'MOODLE_22_STABLE'
 *
 * @package   moodle
...
$string['cookiesenabled_help'] = 'このサイトでは、2種類のクッキーが使用されます。
重要なクッキーはセッションクッキーで、通常 **MoodleSession** と呼ばれます。継続的にログインして各ページに移動するため、ブラウザをクッキー利用可にしてください。 ログアウトまたはブラウザを閉じた場合、(ブラウザとサーバの) クッキーは破棄されます。
もう1つのクッキーは、純粋に便宜的な理由で使用され、通常 **MOODLEID** と呼ばれます。このクッキーは、あなたのユーザ名をブラウザに憶えさせるために使用されます。これは、あなたがサイトへ戻ったときに、ログインページにユーザ名が初めから入力されていることを意味します。このクッキーを拒否しても大丈夫です - しかし、ログイン時にあなたのユーザ名を毎回入力する必要があります。';
...
$string['coursehelpnewsitemsnumber'] = 'コースホームページ右側下部のニュースボックス内に表示される最新ニュースアイテム数です (ゼロはニュースボックスが表示されないことを意味します)。';
...
$string['duplicatenosupport'] = '{$a->modtype} モジュールがバックアップおよびリストアをサポートしていないため、{$a->modname}　活動を重複させることはできません。';
...
$string['expirynotifystudents_help'] = '利用有効期間が設定された場合、この設定では学生がコースから受講登録解除される前にメール通知を受信するかどうか設定します。';
...
$string['forcedmodeinbrackets'] = '{$a} (強制モード)';
...
$string['formattext'] = 'Totaraオートフォーマット';
...
$string['maxnumberweeks'] = '週/トピックの最大数';
$string['maxnumberweeks_desc'] = 'ここではコースの「週/トピックの数」設定に表示されるオプションの最大数をコントロールします。';
...
$string['publicsitefileswarning3'] = '注意: ここに置いたファイルはURIを知っている (または推測できる) 誰でもアクセスすることができます。<br />セキュリティ上の理由から、バックアップファイルは安全なbackupdataフォルダのみに保存してください。';
$string['publish'] = '公開';
$string['question'] = '問題';
...
$string['rsskeyshelp'] = 'セキュリティおよびプライバシーを保証するため、RSSフィードURIにはユーザを識別するための特別なトークンを含みます。これにより、RSSフィード経由でアクセスできないMoodleエリアにユーザがアクセスすることを防ぎます。</p><p>このトークンは、MoodleがRSSフィードを生成する場所にあなたが初回アクセスした時点で自動的に作成されます。あなたがRSSフィードトークンに問題があると感じた場合、リセットリンクをクリックすることで新しいトークンをリクエストすることができます。この場合、あなたの現在のRSSフィードURIが無効になることに留意してください。';
...
$string['showdescription_help'] = 'このオプションを有効にした場合、上記イントロダクション/説明文がコースページ内の活動/リソースのリンクの下に表示されます。';
