...
/**
 * Strings for component 'question', language 'ja', branch 'MOODLE_22_STABLE'
 *
 * @package   question
...
$string['exportquestions_help'] = 'この機能では1つのカテゴリ (およびすべてのサブカテゴリ) の問題すべてをテキストファイルにエクスポートすることができます。
多くのファイルフォーマットで、問題がエクスポートされるときに、情報が失われる可能性があることに注意してください。これは多くのフォーマットが、Moodleの小テスト問題の機能すべてを処理できるわけではないためです。問題がエクスポートできることや、インポートできること、エクスポートしてインポートした問題が同一であることを期待しないでください。また、ある問題タイプは全くエクスポートされない場合もあります。実稼動環境で使用する前に、エクスポートデータをチェックすることをお勧めします。
現在サポートされているフォーマットは下記のとおりです:
## GIFTフォーマット
GIFTはMoodleの小テストをテキストファイルにエクスポートするための、最も包括的なインポート・エクスポートフォーマットです。GIFTは教師が問題をテキストファイルで簡単に書くことができるように設計されています。多肢選択問題、○/×問題、記述問題、組み合わせ問題、数値問題、\_\_\_|\_\_を挿入することで「穴埋め問題」をサポートします。Cloze問題は現在はサポートされていないことに注意してください。
様々なタイプの問題およびコメント、問題名、フィードバック、評点の加重割合を1つのテキストファイルに記述することができます。次に例をあげます:
...
## Moodle XMLフォーマット
このMoodle特有のフォーマットは小テスト問題をシンプルなXMLフォーマットにエクスポートします。エクスポートしたファイルを他の小テストカテゴリにインポートしたり、XSLTのような他の処理に使用することができます。XMLフォーマットでは問題に添付されているイメージを (base64エンコードで) エクスポートします。
## IMS QTI 2.0
標準的なIMS QTI (version 2.0) フォーマットでエクスポートします。この機能ではファイル群を単一のZipファイルに圧縮します。
[IMS QTIフォーマットの詳細情報](http://www.imsglobal.org/question) (外部サイトを別のウィンドウに開きます)
## XHTML
カテゴリを1ページの厳密なXHTMLにエクスポートします。それぞれの問題は明確にタグの中に入れられます。このファイルをそのまま使いたい場合は少なくともタグの最初にあるタグを編集して適切な処理を入れてください。(例 \'mailto\')
インポートおよびエクスポートフォーマットは脱着可能なリソースです。他のオプションフォーマットはモジュールおよびプラグインデータベースで利用することができます。';
...
どのような知識がこの問題で問われているのかという背景に関して、あなたは全般に対するフィードバックを使用して学生に提示することができます。または問題が分からない場合、さらなる情報のリンクを与えて学生が利用できるようにすることもできます。';
...
$string['howquestionsbehave_help'] = '学生は小テスト内の問題に対して、様々に異なる方法で接することができます。例えば、あなたが学生にそれぞれの問題で答えを入力した後、評定またはフィードバックの前に小テストすべてを送信してもらいたい場合もあるでしょう。これには「遅延フィードバック」モードを使用します。代わりに、あなたが学生に即時フィードバックを与えて、仮にすぐに正解できない場合に評点の低い次の受験に挑戦させたいとします。これには「複数受験インタラクティブ」モードを使用します。';
...
$string['includesubcategories'] = 'サブカテゴリからも問題を表示する';
...
$string['matchgrades_help'] = 'インポートされた評点は下記の有効な評点一覧に合致する**必要があります**。
...
上記一覧のマイナス値も使用することができます。
このオプションには2種類の設定があります。これらの設定は上記一覧に**正確に**合致しない値を、インポートルーチンがどのように処理するかに影響します。

\* **|評点が一覧に無い場合、エラーにする**
一覧に無い評点が問題に含まれている場合、エラーが表示され、その問題はインポートされません。
\* **|評点が一覧に無い場合、最も近い評点を使用する**
一覧に無い評点が問題に含まれている場合、評点は一覧の中で一番近い値に変更されます。

*注意: いくつかのカスタムインポートフォーマットではデータベースにデータを直接書き込み、このチェックをバイパスします。*';
...
$string['parentcategory_help'] = '## 親
このカテゴリが配置されているカテゴリです。「トップ」はこのカテゴリが他のカテゴリに含まれていないことを意味します。
通常、いくつかの太字のカテゴリ「コンテクスト」が表示され、それぞれのコンテクストには独自のカテゴリ階層が含まれます。コンテクストに関して、下記関連情報をご覧ください。いくつかのコンテクストが表示されない場合、あなたには他のコンテクストにアクセスするパーミッションがないことが考えられます。
コンテクスト内に少なくとも1つのカテゴリを登録する必要があるため、コンテクスト内に1つのカテゴリのみある場合、あなたはカテゴリを他へ移動することができません。
関連情報 :
* \[問題カテゴリ\]\[1\]
* \[カテゴリコンテクスト\]\[2\]
* \[問題パーミッション\]\[3\]
...
$string['penaltyfactor_help'] = 'ここではそれぞれの間違った解答に対して、差し引く評点の割合を指定することができます。これは問題に繰り返し答えることのできる、アダプティブモードで小テストが動作している場合のみ関係があります。ペナルティ要素は0から1の間に設定してください。ペナルティ要素が１の場合、学生は単位を得るため、最初の解答で正解する必要があります。ペナルティ要素が0の場合、学生は満点になるまで何度でも好きなだけ受験することができます。';
