...
/**
 * Strings for component 'lti', language 'ja', branch 'MOODLE_22_STABLE'
 *
 * @package   lti
...
$string['id'] = 'ID';
$string['imsroleadmin'] = 'インストラクタ、管理者';
$string['imsroleinstructor'] = 'インストラクタ';
$string['imsrolelearner'] = '学習者';
...
$string['lti_launch_error_unsigned_help'] = 'ツールプロバイダのコンシューマーキーおよび共通秘密鍵が不足しているため、このエラーが発生したと考えられます。
コンシューマーキーおよび共通秘密鍵がある場合、あなたは外部ツールインスタンスの設定時に入力することができます (拡張要素が表示されていることを確認してください)。
代わりに、あなたは\[ここで\] [here] ({$a->course_tool_editor}) コースレベルのツールプロバイダ設定を作成することができます。';
...
外部ツールインスタンスを設定するには、ツールプロバイダによるLTI (Learning Tool Interoperability 学習ツール相互運用) のサポートが必要です。あなたがLTIをサポートするツールプロバイダを探し出した場合、そのプロバイダは外部ツールインスタンスの設定方法のインストラクションを提供できるはずです。加えて、サイト管理者により設定されたツールタイプも利用できます。
いくつかの点において、外部ツールはURIリソースと異なります:
\* **コンテクスト志向 (context aware)** - 外部ツールはインスティテューション、コース、氏名および他の情報等、ツールを起動したユーザの情報にアクセスすることができます。
* **細部にわたる統合 (deep integration)** - 外部ツールは活動インスタンスに関わる評定の閲覧、更新および削除をサポートします。将来的なリリースにおいて、さらなる統合が予定されています。
\* **セキュリティ (Security)** - 外部ツール設定では、安全な通信を提供することにより、Moodleおよびツールプロバイダ間の信頼関係を確立します。';
...
$string['send'] = '送信';
$string['setdefault'] = '委任された場合、教師のデフォルト値を設定する';
$string['setupbox'] = 'LTIツールセットアップボックス';
...
$string['toolurl_help'] = 'ツールベースURIは、正しいツール設定をツール起動URIと合致させるために使用されます。URIの接頭辞「http(s)」は任意です。
また、外部ツールインスタンスで起動URIが指定されていない場合、ベースURIが使用されます。
