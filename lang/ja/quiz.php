...
/**
 * Strings for component 'quiz', language 'ja', branch 'MOODLE_22_STABLE'
 *
 * @package   quiz
...
$string['addingquestions'] = 'ここでは、データベース内の問題を管理します。問題は整理しやすいようカテゴリごとに保存されます。また、「公開」を選択した場合、コース内または他のコースの小テストで使用することができます。<br /><br />問題カテゴリの作成を選択した後、問題の作成または編集を行うことができます。ページの反対側にある問題リストから問題を選択して、小テストに追加することができます。';
...
$string['confirmclose'] = '送信した場合、この受験に関して、これ以上あなたの答えを変更することができません。';
...
$string['decimalplaces_help'] = 'この設定では学生の評点を表示または評定するときに、小数点以下を何桁表示するか指定します。この設定は評定の表示のみに影響します。データベースに保存された評点および常に完全精度のため実行される内部計算には影響しません。';
$string['decimalplacesquestion'] = '問題評点の小数位';
$string['decimalplacesquestion_help'] = '個々の問題の評点を表示する場合、小テスト全体の評点に対して、あなたは異なる精度を選択することができます。例えば、個々の問題では分数の評点を使用するのに対して、小テストの最終評点では最も近い整数に丸めたいこともあるでしょう。';
...
$string['emailnotifysmall'] = '{$a->studentname} が {$a->quizname} を完了しました。';
...
全般に対するフィードバックがいつ表示されるか、小テスト設定ページの「学生はレビューできる」チェックボックスでコントロールすることができます。
どのような知識がこの問題で問われているのかという背景に関して、全般に対するフィードバックを使用して学生に提示することができます。または問題が分からない場合、さらなる情報のリンクを与えて学生が利用できるようにすることもできます。';
$string['grade'] = '評点';
...
$string['grademethod_help'] = '複数回の受験が許可されている場合、異なる方法で学生の最終評点を計算することができます。
## 最高評点
すべての受験の中の最高 (最良) 評点が最終評点です。
## 平均評点
最終評点はすべての受験の平均評点 (単純平均) です。
## 最初の受験
最終評点は一番最初の受験で得点した評点です (他は無視されます)。
## 最新の受験
最終評点には直近の受験で得点した評点のみ考慮されます。';
...
$string['modulename_help'] = 'このモジュールでは教師が小テストをデザインおよび設定することができます。小テストには多肢選択問題、○/×問題、組み合わせ問題等、様々なタイプの問題を含むことができます。それぞれの受験は自動的に評定され、教師はフィードバックを投稿および正解表示するかどうか選択することができます。';
...
$string['newpage_help'] = '問題数の多い小テストの場合、1ページあたりの問題数を制限して、小テストを複数のページに分けることは理にかなっています。小テストのページに問題を追加する場合、この設定にしたがって改ページ (Page break) が自動的に挿入されます。後で小テストの編集ページにて、手動で改ページ (Page break) の位置を変えることもできます。';
...
$string['orderandpaging_help'] = 'それぞれの問題の反対側にある番号10、20、30 ... は問題順を意味します。問題を追加できるよう、番号は10ずつ増やされます。問題順を並び替えるには、番号を変更した後、「問題を並び替える」ボタンをクリックしてください。
...
例えば、あなたが下記のように入力したとします:
評点境界: 100%
フィードバック: よくできました。
評点境界: 40%
フィードバック: 今週の課題をもう一度学習してください。
...
評点境界: 0%
この場合、評点が100%から40%の学生にはメッセージ「よくできました。」が表示され、39.9%から0%の学生には他のメッセージが表示されます。つまり、評点境界は評点の幅を決定し、該当する範囲に入る評点に対して、それぞれのフィードバックが表示されます。
評点境界には「31.41%」のようなパーセンテージ、または「7」のような数値を指定することができます。あなたの小テストが最大評点10点で評点境界が7の場合、7/10またはそれ以上を意味します。';
$string['overdue'] = '期限切れ';
...
$string['penaltyscheme_help'] = '小テストがアダプティブモードで動作している場合、学生は間違った解答の後に問題を再受験することができます。この場合、問題の最終評点からペナルティ分の点数を差し引くことができます。ペナルティの点数は設定または編集時にそれぞれの問題で指定します。
小テストがアダプティブモードで動作しない限り、この設定は影響しません。';
...
$string['reorderquestions'] = '問題を並び替える';
...
$string['requiresubnet_help'] = 'このフィールドは任意フィールドです。
LAN上の特定のサブネットまたはインターネットから小テストへのアクセスを、カンマで区切った部分的または完全なIPアドレスで制限することができます。
これは特に特定の部屋の人のみ小テストにアクセスすることができる試験監督付き小テストの実施に便利です。
例えば: **192.168. , 231.54.211.0/20, 231.3.56.211, 231.3.56.10-20** のように指定します。
ここでは4種類の数値を使うことができます (example.comのようなテキストベースのドメイン名は使用できません):
1. **192.168.10.1** のような完全なIPアドレス。1台のコンピュータ (またはプロクシ) と一致します。
2. **192.168** のような部分的なIPアドレス。この数値から始まるコンピュータと一致します。
3. **231.54.211.0/20** のようなCIDR表記法。より詳細なサブネットを指定することができます。
4. **231.3.56.10-20** のようなIPアドレスの範囲。範囲はIPアドレスの最後の部分に適用されます。この場合、231.3.56.10 から 231.3.56.20 の範囲を意味します。
...
ケイパビリティ「自分の非表示の評定を表示する」 [moodle/grade:viewhidden] が設定されたユーザ (通常、教師および管理者) はこれらの設定に影響されず、学生の受験に関するすべての情報を常にレビューすることができます。';
...
$string['showuserpicture_help'] = 'あなたがこのオプションを有効にした場合、受験中およびレビュー時、画面に学生の氏名および写真が表示されます。
1つの例として、試験監督による試験時、ユーザがその人自身でログインしているかどうか確認するため、使用できます。';
...
$string['shufflewithin_help'] = 'このオプションを「Yes」にした場合、学生がこの小テストの受験を開始するたびに、個々の問題の構成要素がランダムにシャッフルされます。この場合、各問題の設定にあるシャッフルオプションも「Yes」にする必要があります。
問題の構成要素をシャッフルする意図は学生同士が答えをコピーし合うことを、少しだけ難しくすることにあります。
このオプションは多肢選択問題や組み合わせ問題のように、複数の構成要素を持つ問題にのみ適用されます。多肢選択問題ではこのオプションが「Yes」にされた場合のみ、答えの順番がシャッフルされます。組み合わせ問題では常に答えがシャッフルされ、この設定は「問題 - 答え」の組み合わせをシャッフルするかどうかコントロールします。
このオプションはランダム問題の利用には関係しません。';
...
$string['timelimit_help'] = 'デフォルトでは小テストに制限時間はなく、学生は小テストを終えるまで好きなだけ時間を使うことができます。また、学生は終了日以前に小テストを終了することもできます。
あなたが時間制限を設定した場合、画面上にカウントダウンクロックが表示されます。また、時間切れになった場合、小テストは自動的に送信されます。時間制限が設定された場合、学生は制限時間前に受験を送信する必要があります。制限時間を超えて送信した場合、学生は評点を得ることができません。';
