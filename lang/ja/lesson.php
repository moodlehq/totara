...
/**
 * Strings for component 'lesson', language 'ja', branch 'MOODLE_22_STABLE'
 *
 * @package   lesson
...
$string['casesensitive_help'] = 'チェックボックスをチェックした場合、解答の分析に正規表現を使用します。';
...
$string['importppt_help'] = '<p>使用方法</p>
<p>すべてのPowerPointスライドは、前および次の答えを持ったブランチテーブルとしてインポートすることができます。</p>
<ol>
<li>PowerPointプレゼンテーションを開いてください。</li>
<li>ウェブページ (特別オプションなし) として保存してください。</li>
<li>すべてのスライドがウェブページにコンバートされ、htmlファイルおよびフォルダが作成されます。フォルダをzip圧縮してください。</li>
<li>あなたのmoodleサイトにアクセスして、新しいレッスンを追加してください。</li>
<li>レッスン設定の保存後、「最初に何をしますか?」の下に4つのオプションが表示されますので、「PowerPointのインポート」をクリックしてください。</li>
<li>「Browse...」ボタンをクリックして、ステップ3で作成したzipファイルを指定してください。そして、「ファイルをアップロードする」をクリックしてください。</li>
<li>処理が正常に行われた場合、次の画面では「続ける」ボタンのみ表示されます。</li>
</ol>
<p>PowerPointにイメージが含まれる場合、イメージはコースファイルとしてmoddata/XYに保存されます。Xはレッスン名、Yはナンバー (通常 0) です。また、インポート処理中、moodleデータディレクトリtemp/lessonの中にファイルが作成されます。これらのファイルは、現在のところimportppt.phpで削除されることはありません。</p>
<p align="center">&nbsp;</p>';
...
$string['modulename_help'] = '<p><img alt="" src="<?php echo $CFG->wwwroot?>/mod/lesson/icon.gif" />&nbsp;<b>レッスン</b></p>
...
$string['pptsuccessfullimport'] = 'アップロードされたPowerPointプレゼンテーションよりページを正常にインポートしました。';
...
$string['progressbar_help'] = '<p>レッスンの下部に、プログレスバーを表示します。現在、プログレスバーは、直線のレッスンに対して最も誤差がありません。</p>
<p>パーセントの計算が完了する間、正しく答えられたブランチテーブルおよび問題ページは、レッスンの進捗に加算されます。レッスンの合計ページが計算される間、クラスタおよびクラスタ内のページは単一のページとして計算され、終了クラスタおよび終了ブランチは除外されます。他のすべてのページは、レッスンの総ページ数に加算されます。</p>
<p>注意: プログレスバーのデフォルトスタイルの見栄えはよくありません。プログレスバーのすべてのスタイル (例  色、背景イメージ等) は、mod/lesson/styles.php で変更することができます。</p>';
