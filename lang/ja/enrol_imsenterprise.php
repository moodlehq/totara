...
/**
 * Strings for component 'enrol_imsenterprise', language 'ja', branch 'MOODLE_22_STABLE'
 *
...
<p>コースは最初に、(例えば) 学生情報システム内のコースを特定するために使用できるコード、Moodleコーステーブルの英数字フィールド「idnumber」で検索されます。「idnumber」が見つからない場合、コーステーブルはブレッドクラム等で表示されるMoodleのコース省略名「short description」で検索されます。(いくつかのシステムではこれら2つのフィールドはおそらく同じです。) 検索が失敗した場合のみ、プラグインは任意で新しいコースを作成することができます。</p>
