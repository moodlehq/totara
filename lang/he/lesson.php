...
<p>בכל אופן, ניתן להשתמש במודול השיעור כסוג של מטלת <i> שקופית "פלאש" </i>.לסטודנט מוצגים מידע (אופציונלי) ושאלה בסדר רנדומלי בד"כ. אין קביעה של התחלה או קביעה של סוף. רק מוצגות קביעות של <i>שקופיתים</i> אחד אחרי השני בסדר שאיננו מיוחד.
</p>
<p>אפשרות זו מותירה לשתי גרסאות דומות מאוד של התנהגות שקופיות "פלאש". האפשרות "הצג עמוד שלא נראה" לעולה לא מציגה את אותו עמוד פעמיים (גם אם הסטודנט <b>לא</b> ענה על השאלה הקשורה לשקופיתעמוד הנוכחי). האפשרות השניה (שאיננה ברירת מחדל) "הצג עמוד שלא נענה" מאפשר לסטודנט לצפות עמודים אשר כנראה הופיעו בעבר אך רק אם הם ענו על השאלה המקושרת בצורה שגויה.</p>

<p>בשקופית ה-"פלאש" וגם בסוג השיעור המורה יכול להחליט אם להשתמש בכל השקופיותעמודים שבשיעור או רק בתת-קביעה (רנדומלית). הדבר נעשה ע"י פרמטר " מספר העמודים (שקופיות) להצגה "</p>';
...
<li>
  <p><b>תשובה קצרה</b> ישנן שתי מערכות השוואה שונות שניתן להשתמש בהן לשאלות מסוג \'תשובה קצרה\': המערכת הפשוטה ביותר משמשת כברירת המחדל. ניתן להשתמש במערכת ה"ביטוי הרגולרי", כל שצריך לעשות זה לסמן את תיבת הסימון של האפשרות "ביטוי רגולרי". למידע נוסף, אנא קיראו את קובץ העזרה: סוגי שאלות בשיעור.
</p>
</li>
</ol>
...
$string['displaydefaultfeedback_help'] = '<p align="center"><strong>הצג את המשוב שמשמש כברירת המחדל</strong></p>

<p>אם אפשרות זו נקבעת ל<strong>כן</strong>, אז, גם כאשר לא נמצאה תגובה לשאלה מסויימת, ישתמשו בתגובות שמשמשות כברירות המחדל: "זוהי התשובה הנכונה" ו"זוהי לא התשובה הנכונה". </p>
<p>אם אפשרות זו נקבעת ל<strong>לא</strong>,אז כאשר לא נמצאה תשובה לשאלה מסויימת, לא יוצג עליה משוב. הסטודנט שלומד את השיעור יופנה אוטומטית לעמוד הבא בשיעור.
</p>';
...
$string['displayleftmenu_help'] = '<p>אפשרות זו תציג רשימה של העמודים (דף תוכן מסתעף) בשיעור זה.</p>
...
$string['importppt_help'] = '<p align="center"><b>ייבוא PowerPoint HTML</b></p>
<p> איך להשתמש</p>
<p>כל שקופיות PowerPoint מיובאות כטבלאות הסתעפויות (Branch Tables) עם תשובות הקודמות להן ותשובות הבאות לאחריהן.
</p>
<p>
<ol>
<li>פתחו את מצגת ה PowerPoint שלכם.</li>
<li>שימרו אותה כעמוד רשת (ללא אפשרויות מיוחדות).</li>
<li>התוצאה של השלב השלישי צריכה להיות קובץ html ותיקייה שמכילה את כל השקופיות כשהן מומרות לעמודי רשת.<br />
כווצו התיקייה בלבד באמצעות תוכנת ZIP.
</li>
<li>לכו לאתר המוודל שלכם והוסיפו שיעור חדש. </li>
<li>לאחר ששמרתם את הגדרות השיעור, צריכות להופיע בפניכם 4 אפשרויות תחת הכותרת &quot;מה תרצו לעשות קודם?&quot;, ליחצו על &quot;לייבא את PowerPoint&quot;.
</li>
<li>השתמשו בכפתור ה&quot;דפדף...&quot; כדי למצוא את קובץ ה-ZIP שיצרתם בשלב 3. לאחר שמצאתם אותו ליחצו על &quot;העלה את הקובץ הזה&quot;.
</li>
<li>אם הכל עבד כמו שצריך, במסך הבא אמור להופיע בפניכם כפתור \'המשך\'.
</li>
</ol>
</p>
<p>אם היו לכם תמונות כלשהן ב-PowerPoint שלכם, הן היו אמורות להשמר כקבצי קורס ב-moddata/XY, כאשר ה-X הוא שם הקורס שלכם וה-Y הוא מספר (בדרך כלל 0). בנוסף לכך, במהלך תהליך הייבוא, נוצרים קבצים בתוך תיקיית נתוני מוודל שלכם, בתוך: temp/lesson. נכון לעכשיו, קבצים אלה לא נמחקים על ידי importppt.php.
</p>
<p align="center">&nbsp;</p>';
...
$string['jump'] = 'קפיצה';
$string['jumps'] = 'קפיצות';
$string['jumps_help'] = '<p align="center"><b>קישור ה"קפוץ ל"</b></p>

<p>לכל תשובה (לשאלה) או תיאור (לעמודי הסתעפויות) מחובר קישור \'קפוץ ל\'. כאשר התשובה נבחרה, תגובת התשובה מוצגת לסטודנט. לאחר מכן, הסטודנט נשלח לעמוד שנקבע בקישור ה\'קפוץ ל\'. הקישור יכול להיות יחסי או מוחלט. קישורים יחסיים הם  <b>העמוד הזה</b> ו<b>העמוד הבא</b> .<b>העמוד הזה</b> משמעותו שהסטודנט רואה שוב את העמוד הנוכחי. <b>העמוד הבא</b> מראה את העמוד הבא העוקב אחר העמוד הזה בסדר הלוגי של העמודים. קישור מוחלט (לעמוד) נקבע על ידי בחירה ב<b>כותרת</b> של העמוד.</p>

<p>שימו לב שקישור \'קפוץ ל\' <b>עמוד הבא</b> (קישור יחסי), יכול להפנות לעמוד שונה לאחר שהעמודים שינו מקום. זאת לעומת קישור \'קפוץ ל\' שמשתמש ב<b>כותרות</b> של עמודים, ולכן תמיד יראו את אותו העמוד גם לאחר שהעמודים עברו מקום.
</p>
<p>קפיצות מיוחדות</p>
<p>שאלת "אנסין" (קטע טקסט שלא נלמד) בתוך הסתעפות</p>
<p>אפשרות זו תקשר לשאלת "אנסין" שנבחרה באופן שרירותי (על ידי הסטודנט בניסיון זה) בין טבלת הסתעפות זו וסוף השיעור או \'סוף ההסתעפות\' הבא.
</p>
...
<p>אפשרות זו תקפוץ באופן שרירותי לטבלת הסתעפות שנמצאת בין טבלת ההסתעפות הנוכחית וסוף השיעור, או \'סוף ההסתעפות\' הבא.
</p>';
...
<p>ערך זה קובע את מספר התשובות המקסימילי בהן יכול המורה להשתמש. ערך ברירת המחדל הוא 4. אם השיעור עושה שימוש בלעדי ב, נגיד, שאלות נכוןלא נכון לכל אורכו, אז הגיוני לקבוע את הערך הזה ל-2.
</p>

<p>פרמטר זה קובע גם כן את המספר המירבי של הסתעפויות בהן ניתן להשתמש בדף תוכן מסתעף.
</p>

<p>את הפרמט הזה ניתן לשנות בביטחה בכל שיעור בעל תוכן קיים. למעשה, במקרה ותרצו להוסיף שאלה מרובת אפשרויות או דף תוכן מסתעף ארוך, אתם תהיו חייבים לשנות את הפרמטר הזה. לאחר שהוספתם את השאלה (החריגה) או את טבלת ההסתעפויות (BranchTable), אתם יכולים להנמיך את הערך הזה למשהו מעט יותר "סטנדרטי".
...
$string['modulename_help'] = '<p><img alt="" src="<?php echo $CFG->wwwroot?>/mod/lesson/icon.gif" />&nbsp;<b>שיעור</b></p>

<div class="indent">
השיעור מעביר תוכן בצורה מעניינת וגמישה. הוא מורכב ממספר עמודים. כשבדרך כלל, כל עמוד מסתיים בשאלה בעלת מספר תשובות אפשריות. לפי התשובה שהסטודנט נותןהוא, או ממשיך לעמוד הבא או, נלקח חזרה לעמוד הקודם.
...
$string['multianswer_help'] = '<p align="center"><b>אפשרויות של שאלות</b></p>
...
<li><p><b>שאלות אמריקאיות (שאלות רבות בחירה)</b>ישנה גירסא לשאלות האמריקאיות (רבות בחירה) הנקראת שאלות
<b>&quot;אמריקאיות (רבות בחירה), מרובות תשובות&quot;</b>. אם אתם בוחרים באפשרות זו, אז הסטודנט מחוייב לבחור בכל התשובות הנכונות מתוך מקבץ התשובות. בשאלה עצמה לאוו דווקא יאמר לסטודנט <i>כמה</i> תשובות נכונות ישנן. לדוגמא, השאלה: &quot;מי מהאנשים הבאים כיהן כנשיא ארה"ב??&quot; לא מודיעה לסטודנט כמה תשובות נכונות ישנן, לעומת השאלה: "בחרו מהרשימה הבאה את שני האנשים שכיהנו כנשיאי ארה"ב", שכן עושה זאת. מספר התשובות הנכונות יכול להיות כל ערך הכל מ<b>אחת</b> ועד למספר של כל האפשרויות שמתוכן ניתן לבחור.
{שאלה אמריקאית \'מרובת תשובות\' בעלת רק תשובה נכונה אחת <b>הינה</b> עדיין שונה משאלה אמריקאית (רבת בחירה) בגלל שהיא עדיין מאפשרת לסטודנט לבחור ביותר מתשובה אחת, בעוד שהשאלה האמריקאית (רבת הבחירה) לא מאפשרת זאת כלל.}
</p></li>
...
<li>לשאלות מאמר, אין תשובה, ישנם רק תוצאה, משוב ו\'קפיצת\' עמוד.</li>
<li>לכל תשובה יכול להיות קטע טקסט קצר שמוצג אם וכאשר התשובה נבחרת. קטע הטקסט נקרא <b>תגובה</b>.</li>
<li> - בנוסף לכך, לכל תשובה מותאמת <b>\'קפיצה\'</b>. ה\'קפיצה\' יכולה להיות יחסית - לעמוד הזה, לעמוד הבא - או מוחלטת - מציינת כל אחד מהעמודים בשיעור או את סוף השיעור. </li>
...
סדר זה נקבע על ידי ה\'קפיצות\' שמוגדרות לשאלות פרטניות, והוא יכול להיות שונה ביותר מהסדר הלוגי, אלא אם כן, <i>לא</i> משנים את ערכי ה\'קפיצות\' ומשאירים אותם לפי ערכי ברירת המחדל (השניים קשורים באדיקות אחד לשני). בידי המורה האפשרות לבדוק את סדר הניווט.
</li>
...
תשובות <b>לא נכונות</b>, הן אלו ש\'קופצות\' או, לאותו העמוד או, לעמוד שנמצא <i>למעלה</i> מהעמוד הנוכחי בסדר הלוגי. כך שאם אתם <i>לא</i> משנים את ה\'קפיצות\', התשובה הראשונה היא התשובה הנכונה, ושאר התשובות הן תשובות לא נכונות.
    <br /><br />
...
<p>הצג סרגל התקדמות בתחתית השיעור. כרגע, סרגל התקדמות זה מציג באופן הכי מדוייק את ההתקדמות הנעשית בשיעור הליניארי. </p>
