...
$string['availabletags_help'] = '<p align="center"><strong>תגים זמינים</strong></p>
<p>תגים הם מסמני מקום בתוך התבנית, הם מוחלפים על ידי שדות או כפתורים כשהרשומות נצפות או עוברות עריכה.
</p>
<p>לשדות יש את התסדיר הבא: [[fieldname]]</p>
<p>לכפתורים יש את התסדיר הבא: ##somebutton##</p>
<p>בשביל התבנית הנוכחית אפשר להשתמש רק בתגים הנמצאים ברשימת התגים זמינים.
</p>';
...
$string['csvimport_help'] = '<p align="center"><strong>ייבוא מקובץ CSV</strong></p>

<p>CSVהוא תסדיר נפוץ לחליפין של טקסט, ופירושו: ערכים המופרדים על ידי פסיקים (Comma-Separated-Values). </p>

<p>תסדיר הקובץ הצפוי הוא תסדיר טקסט פשוט (plain text) כאשר ברשומה הראשונה ישנה רשימת שמות של שדות. הנתונים באים לאחר מכן, לכל שורה רשומה אחת.
</p>

<p>תוחם השדות חוזר לברירת המחדל שהיא תו של פסיק. סוגר שטח השדה לא נקבע על ידי ברירת מחדל (סוגרי השטח של השדות הם תווים שמקיפים כל שדה בכל רשומה).

</p>

<p>רשומות צריכות להיות מופרדות על ידי קווים חדשים (שלרוב מחוללים על ידי לחיצה על כפתור ה ENTER או ה RETURN בעורך הטקסט שלכם). בטאבים (Tabs) אפשר לנקוב באמצעות t ובשורות חדשות באמצעות n.
...
<p>אזהרה: יכול להיות שאין תמיכה מלאה לכל סוגי השדות.
</p>';
...
$string['exportaszip_help'] = '<p align="center"><strong>יצוא כקובץ Zip</strong></p>
<p> הדבר מאפשר לכם להוריד את התבניות למחשב האישי שלכם. לאחר מכן, תוכלו העלות אותם לבסיס נתונים אחר, באמצעות אופציית הייבוא של Zip.
</p>';
...
$string['fieldmappings_help'] = '<p align="center"><strong>מיפוי שדות</strong></p>
<p>התפריט הזה מאפשר לכם לשמור את הנתונים הקיימים בבסיס הנתונים הנוכחי. כדי לשמר את הנתונים האלה בתוך שדה, אתם חייבים למפות אותו לשדה חדש שם יופיעו הנתונים. אתם יכולים גם להשאיר שדות ריקים, מבלי להעתיק לתוכם נתונים. שדה ישן שלא מיפיתם אותו לשדה חדש ייאבד וכל הנתונים שלו יימחקו.
</p>
<p>
...
$string['fromfile_help'] = '<p align=\'center\'><strong>ייבוא הגדרות קבועות מראש מתוך קובץ Zip</strong></p>
<p>השתמשו בזה כדי להעלות הגדרות קבועות מראש שנשמרו על המחשב שלכם באמצעות תכונת היצוא.
</p>';
...
$string['maxentries_help'] = '<p align="center"><strong>מקסימום רשומות</strong></p>

<p>מספר הרשומות המירבי שמשתתף יכול להגיש לפעילות זו.
</p>';
...
$string['newfield_help'] = '<p align="center"><strong>שדות</strong></p>

<p>במסך הזה אתם יכולים ליצור את השדות שיהוו חלק מבסיס הנתונים שלכם.
</p>

<p> כל שדה מאפשר סוגי נתונים שונים, עם ממשקים שונים.
</p>';
...
$string['requireapproval_help'] = '<p align="center"><strong>נדרש אישור</strong></p>

<p>האם רשומות צריכות לעבור אישור של מורה לפני שהן מוצגות בפני הסטודנטים? האפשרות הזו מועילה למיתון תוכן שיכול להיות פוגעני או לא הולם.
</p>';
$string['requiredentries'] = 'מספר הרשומות הנדרשות';
$string['requiredentries_help'] = '<p align="center"><strong>מספר הרשומות הנדרשות</strong></p>

<p>מספר הרשומות שמשתתף נדרש להגיש. בפני משתתפים שעדיין לא הגישו את מספר הרשומות הנדרשות, תופיע הודעה שנועדה להזכיר להם את חובת ההגשה שלהם.
</p>

<p>עד שהמשתמש יגיש את כל הרשומות הנדרשות, הפעילות לא תיחשב כגמורה.
</p>';
$string['requiredentriestoview'] = 'מספר הרשומות הנדרשות לפני שתתאפשר צפיה';
$string['requiredentriestoview_help'] = '<p align="center"><strong>רשומות הנדרשות לפני צפיה</strong></p>

<p>מספר הרשומות שמשתתף מחוייב להגיש לפני שהוא מורשה לצפות ברשומות הקיימות בבסיס הנתונים של פעילות זו.
</p>
<p>הערה: לא ניתן להשתמש באפשרות זו יחד עם מסנן הקישורים האוטומטי של מסד הנתונים, מפני שעבור מסנן הקישורים האוטומטי של מסד הנתונים אין אפשרות לקבוע האם משתמש הגיש את מספר נדרש של רשומות. </p>';
...
$string['saveaspreset_help'] = '<p align="center"><strong>שמירת תבנית־שדות</strong></p>

<p>זה מפרסם את התבניות הנוכחיות כהגדרות קבועות מראש וכל מי שנמצא באתר יכול לראות אותן ולעשות בהם שימוש. הן יופיעו ברשימת ההגדרות הקבועות מראש, אתם תוכלו להסיר אותן בכל עת.
</p>';
...
$string['usestandard_help'] = '<p align="center"><strong>השתמש בהגדרה קבועה מראש</strong></p>
<p> משתמש בתבנית שזמינה לאתר כולו.</p>
<p>בנוסף לכך, אם הוספתם את ההגדרה הקבועה מראש לסיפריה באמצעות תכונת ה\'שמור הגדרה קבועה מראש\', תוכלו גם למחוק אותה.
</p>';
