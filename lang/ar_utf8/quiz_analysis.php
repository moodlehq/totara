<?php
// quiz_analysis.php - created with Totara langimport script version 1.1

$string['analysis'] = 'تحليل البند';
$string['analysisdownload'] = 'تنزيل التحليل';
$string['analysisoptions'] = 'خيارات التحليل';
$string['analysistitle'] = 'جدول تحليل البند';
$string['attemptsall'] = 'كل المحاولات';
$string['attemptselection'] = 'المحاولات لتحليل كل مستخدم';
$string['attemptsfirst'] = 'المحاولة الأولى';
$string['attemptshighest'] = 'الدرجة العليا';
$string['attemptslast'] = 'المحاولة الأخيرة';
$string['dicsindextitle'] = 'Disc.<br />Index';
$string['disccoefftitle'] = 'Disc.<br />Coeff.';
$string['downloadooo'] = 'قم بتنزيل الملف في صياغة  OpenOffice';
$string['facilitytitle'] = '%% Correct<br />Facility';
$string['lowmarkslimit'] = 'لا تقم بالتحليل لو كانت الدرجة أقل من :';
$string['pagesize'] = 'الأسئلة في كل صفحة:';
$string['qcounttitle'] = 'عدد الأسئله';
$string['qidtitle'] = 'رقم السؤال';
$string['qnametitle'] = 'اسم السؤال';
$string['qtexttitle'] = 'نص السؤال';
$string['qtypetitle'] = 'نوع السؤال';
$string['quizreportdir'] = 'تقرير الامتحان';
$string['rcounttitle'] = 'R.<br />Counts';
$string['reportanalysis'] = 'تقرير تحليل البند';
$string['responsestitle'] = 'نص الاجابة';
$string['rfractiontitle'] = 'partial<br />credit';
$string['rpercenttitle'] = 'R.%%';
$string['stddevtitle'] = 'SD';

?>
