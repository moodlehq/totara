<?PHP // $Id$ 
      // organisation.php - created with Moodle 1.9.14 (Build: 20111010) (2007101591.06)


$string['adddepthlevel'] = 'Neues Level hinzufügen';
$string['addedframework'] = 'Das Organisationsprofile \"{$a}\" wurde hinzugefügt';
$string['addedorganisation'] = 'Die Organisation \"{$a}\" wurde hinzugefügt';
$string['addmultipleneworganisation'] = 'Mehrere Organisationen hinzufügen';
$string['addnewframework'] = 'Neues Organisationsprofil anlegen';
$string['addneworganisation'] = 'Neue Organisation anlegen';
$string['addtype'] = 'Neuen Typ anlegen';
$string['backtoallframeworks'] = 'Zurück zu allen Organisationsprofilen';
$string['bulkdeleteorganisation'] = 'Organisationen im Bulk löschen';
$string['bulkmoveorganisation'] = 'Organisationen im Bulk verschieben';
$string['chooseorganisation'] = 'Organisation auswählen';
$string['competencyassigndeletecheck'] = 'Sind Sie sicher, dass Sie diese Kompetenzzuweisung löschen möchten?';
$string['createtype'] = 'Der Organisationstyp \"{$a}\" wurde erstellt';
$string['currentlyselected'] = 'Ausgewählt';
$string['deletecheck'] = 'Sind Sie sicher, dass Sie diese Organisation, all Ihre Kindelemente und Daten löschen möchten?';
$string['deletecheck11'] = 'Sind Sie sicher, dass die Organisation \"$a\" gelöscht werden soll?
<br /><br />
Die folgenden Daten werden gelöscht:<br />
- Organisation \"$a\"';
$string['deletecheckframework'] = 'Sind Sie sicher, dass Sie das Profil \"$a\" löschen möchten?';
$string['deletecheckwithchildren'] = 'Sind Sie sicher, dass die Organisation \"$a->itemname\" und $a->children_string gelöscht werden sollen?
<br /><br />
Die folgenden Daten werden gelöscht: <br />
- Organisation \"$a->itemname\" und $a->childcount $a->children_string';
$string['deletedassignedcompetency'] = 'Die Zuweisung der Kompetenz wurde erfolgreich von dieser Organisation ausgelöst';
$string['deletedepth'] = '$a löschen';
$string['deletedframework'] = 'Das Organisationsprofil $a und seine Daten wurden vollständig gelöscht';
$string['deletedorganisation'] = 'Die Organisation $a und ihre Kindelemente wurden vollständig gelöscht';
$string['deletedtype'] = 'Die Organisationstyp \"{$a}\" wurde vollständig gelöscht';
$string['deleteframework'] = '$a löschen';
$string['deleteincludexcustomfields'] = '- $a benutzerdefinierte Felder Einträge';
$string['deleteincludexlinkedcompetencies'] = '- $a Links zu Kompetenzen';
$string['deleteincludexposassignments'] = '- $a Verknüpfung zu dieser Organisation (Verknüpfungen von Benutzern mit dieser Organisation werden aufgehoben)';
$string['deletemulticheckwithchildren'] = 'Sind Sie sicher, dass $a->num Organisation(en) und $a->childcount $a->children_string gelöscht werden sollen?
<br /><br />
Die folgenden Daten werden gelöscht: <br />
- $a->num Organisation(en) und $a->childcount $a->children_string';
$string['deleteorganisation'] = 'Organisation löschen';
$string['deletetype'] = 'Typ \"{$a}\" löschen';
$string['depthlevel'] = 'Level';
$string['depthlevels'] = 'Level';
$string['depths'] = 'Level';
$string['descriptionview'] = '$a Beschreibung';
$string['editdepthlevel'] = 'Level ändern';
$string['editframework'] = 'Organisationsprofil ändern';
$string['editgeneric'] = '$a ändern';
$string['editorganisation'] = 'Organisation ändern';
$string['edittypelevel'] = 'Typ ändern';
$string['error:addorganisation'] = 'Beim Hinzufügen der Organisation \"{$a}\" ist ein Problem aufgetreten';
$string['error:createtype'] = 'Beim Hinzufügen der Organisation \"{$a}\" ist ein Fehler aufgetreten';
$string['error:deleteassignedcompetency'] = 'Beim Entfernen der Kompetenz von dieser Organisation ist ein Fehler aufgetreten';
$string['error:deletedframework'] = 'Fehler beim Löschen des Organisationsprofils $a und den dazugehörigen Daten.';
$string['error:deletedtype'] = 'Beim Löschen des Organisationsprofils \"{$a}\" ist ein Fehler aufgetreten.';
$string['error:dialognotreeitems'] = 'Keine Organisationen in diesem Profil';
$string['error:updateorganisation'] = 'Beim Ändern des Organisationsprofils \"{$a}\" ist ein Fehler aufgetreten.';
$string['error:updatetype'] = 'Beim Ändern des Organisationstyps \"{$a}\" ist ein Fehler aufgetreten.';
$string['featureplural'] = 'Organisationen';
$string['framework'] = 'Organisationsprofil';
$string['frameworks'] = 'Organisationsprofile';
$string['fullname'] = 'Organisation vollständiger Name';
$string['fullnamedepth'] = 'Level vollständiger Name';
$string['fullnameframework'] = 'Vollständiger Name';
$string['fullnametype'] = 'Typ vollständiger Name';
$string['fullnameview'] = 'Vollständiger Name';
$string['idnumber'] = 'Organisation ID';
$string['idnumberframework'] = 'ID';
$string['idnumberview'] = '$a ID';
$string['invalidnumeric'] = 'Der numerische Wert muss numerisch (oder leer) sein.';
$string['itemstoadd'] = 'Elemente hinzufügen';
$string['manageorganisation'] = 'Organisationen verwalten';
$string['manageorganisations'] = 'Organisation verwalten';
$string['manageorganisationtypes'] = 'Typen verwalten';
$string['missingfullname'] = 'Fehlende Organisation vollständiger Name';
$string['missingfullnamedepth'] = 'Fehlendes Level vollständiger Name';
$string['missingfullnameframework'] = 'Fehlendes Profil vollständiger Name';
$string['missingfullnametype'] = 'Fehlender Typ vollständiger Name';
$string['missingname'] = 'Fehlender Organisationsname';
$string['missingnameframework'] = 'Fehlendes Organisationsprofil Name';
$string['missingnametype'] = 'Fehlender Organisationstyp Name';
$string['missingshortname'] = 'Fehlende Organisation Kurzname';
$string['missingshortnamedepth'] = 'Fehlendes Level Kurzname';
$string['missingshortnameframework'] = 'Fehlendes Profil Kurzname';
$string['missingshortnametype'] = 'Fehlender Typ Kurzname';
$string['name'] = 'Name';
$string['nochildorganisations'] = 'Keine Tochterorganisationen definiert';
$string['nodepthlevels'] = 'Keine Level in diesem Profil';
$string['noframeworks'] = 'Keine Organisationsprofile vorhanden';
$string['noframeworkssetup'] = 'Für diese Seite existieren keine Organisationsprofile';
$string['noorganisation'] = 'Keine Organisation definiert';
$string['noorganisationsinframework'] = 'Keine Organisationen in diesem Profil';
$string['notypes'] = 'Keine Organisationstypen';
$string['nounassignedcompetencies'] = 'Keine nicht zugewiesenen Kompetenzen';
$string['nounassignedcompetencytemplates'] = 'Keine nicht zugewiesenen Kompetenzvorlagen';
$string['numericalvalue'] = 'Numerischer Wert';
$string['organisation'] = 'Organisation';
$string['organisationaddnew'] = 'Neue Organisation hinzufügen';
$string['organisationcustomfields'] = 'Benutzerdefinierte Felder';
$string['organisationdepthcustomfields'] = 'Organisationslevel benutzerdefinierte Felder';
$string['organisationframework'] = 'Organisationsprofil';
$string['organisationframeworkmanage'] = 'Profile verwalten';
$string['organisationframeworks'] = 'Organisationsprofile';
$string['organisationmanage'] = 'Organisationen verwalten';
$string['organisationplural'] = 'Organisationen';
$string['organisations'] = 'Organisationen';
$string['organisationtypecustomfields'] = 'Organisationstypen benutzerdefinierte Felder';
$string['organisationtypes'] = 'Organisationstypen';
$string['parent'] = 'Mutterorganisation';
$string['returntoframework'] = 'Zurück zum Organisationsprofil';
$string['shortname'] = 'Organisation Kurzname';
$string['shortnamedepth'] = 'Level Kurzname';
$string['shortnameframework'] = 'Kurzname';
$string['shortnametype'] = 'Typ Kurzname';
$string['shortnameview'] = 'Kurzname';
$string['type'] = 'Type';
$string['types'] = 'Typen';
$string['updatedframework'] = 'Das Organisationsprofil \"{$a}\" wurde geändert';
$string['updatedorganisation'] = 'Die Organisation \"{$a}\" wurde geändert';
$string['updatetype'] = 'Der Organisationstyp \"{$a}\" wurde geändert';
$string['weight'] = 'Gewicht';
$string['organisationbulkaction'] = 'Bulk Aktionen'; // ORPHANED

?>
