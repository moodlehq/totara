<?php
// repository_mahara.php - created with Totara langimport script version 1.1

$string['configplugin'] = 'Configurazione Repository Mahara';
$string['connectionfailure'] = 'Non è stato possibile ottenere l\'elenco dei file. E\' possibile che l\'host Moodle non sia alla versione 2.0 oppure che il servizio Repository Mahara non sia stato attivato';
$string['emptyfilelist'] = 'Non ci sono file da visualizzare';
$string['failtoretrievelist'] = 'Non è stato possibile ottenere l\'elenco oppure l\'elenco è vuoto';
$string['hostnotfound'] = 'Non è stato possibile trovare il sito Mahara nel database. Contattate il vostro amministratore';
$string['mahara:view'] = 'Visualizzare repository Mahara';
$string['nopeer'] = 'Per favore attivate alcuni <a href=\"http://docs.moodle.org/en/admin/mnet/peers#Peer_to_Peer_Network\">Moodle peer</a> per il vostro sito Mahara.';
$string['notitle'] = 'senza titolo';
$string['peer'] = 'Peer';
$string['remember'] = 'Ricordami';
$string['remoterep_description'] = 'Abilita il \"discovery\" del servizio';
$string['remoterep_name'] = 'Mahara';
$string['repositorydesc'] = 'File su server Mahara remoto';
$string['repositoryname'] = 'Mahara';
$string['usercannotaccess'] = 'Tu ($a) non puoi accedere a questo file';
$string['usernotfound'] = 'L\'utente $a non ha un account su Mahara';

?>
