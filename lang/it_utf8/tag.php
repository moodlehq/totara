<?PHP // $Id$ 
      // tag.php - created with Moodle 1.9.14 (Build: 20111010) (2007101591.06)


$string['addedotag'] = '\"$a\" è stata aggiunta come una tag ufficiale.';
$string['addotags'] = 'Aggiungi tag ufficiali';
$string['addtagtomyinterests'] = 'Aggiungi \"$a\" ai miei interessi';
$string['blocktagstitle'] = 'Tag';
$string['changename'] = 'Modifica nome tag';
$string['changetype'] = 'Modifica tipo tag';
$string['count'] = 'Qta';
$string['coursetagging'] = 'Tag del corso';
$string['coursetagginghelp'] = 'Abilita questa opzione per consentire il tagging del corso.';
$string['delete'] = 'Cancella';
$string['deleted'] = 'Cancellato';
$string['description'] = 'Descrizione';
$string['edittag'] = 'Modifica questa tag';
$string['flag'] = 'Flag';
$string['flagasinappropriate'] = 'Marca come non appropriata';
$string['helprelatedtags'] = 'Tag correlate separate da virgola';
$string['id'] = 'id';
$string['managetags'] = 'Gestisci tag';
$string['name'] = 'Nome tag';
$string['namesalreadybeeingused'] = 'Nomi tag già usati';
$string['newname'] = 'Nuovo nome tag';
$string['noresultsfor'] = 'Nessun risultato per \"$a\"';
$string['officialtag'] = 'Tag ufficiale';
$string['otags'] = 'Tag ufficiali';
$string['owner'] = 'Proprietario';
$string['ptags'] = 'Tag definite dall\'utente (separate da virgole)';
$string['relatedblogs'] = 'Interventi blog recenti con questa tag';
$string['relatedtags'] = 'Tag correlate';
$string['removetagfrommyinterests'] = 'Rimuovi \"$a\" dai miei interessi';
$string['reset'] = 'Reset';
$string['resetflag'] = 'Reset flag';
$string['responsiblewillbenotified'] = 'La persona responsabile sarà avvertita';
$string['search'] = 'Cerca';
$string['searchresultsfor'] = 'Cerca risultati per \"$a\"';
$string['searchtags'] = 'Cerca tag';
$string['seeallblogs'] = 'Vedi tutti i blog con questa tag';
$string['select'] = 'Seleziona';
$string['tag'] = 'Tag';
$string['tagdescription'] = 'Descrizione tag';
$string['tags'] = 'Tag';
$string['tagsaredisabled'] = 'Le tag sono disabilitate';
$string['tagtype'] = 'Tipo tag';
$string['tagtype_default'] = 'Default';
$string['tagtype_official'] = 'Ufficiale';
$string['thingstaggedwith'] = '$a->count cose con tag \"$a->name\"';
$string['thistaghasnodesc'] = 'Questa tag al momento non ha descrizione';
$string['timemodified'] = 'Modificata';
$string['typechanged'] = 'Tipo tag modificato';
$string['updated'] = 'Aggiornato';
$string['updatetag'] = 'Aggiorna';
$string['userstaggedwith'] = 'Utenti con il tag \"$a\"';
$string['withselectedtags'] = 'Per le tag scelte ...';
$string['deletedcoursetags'] = 'Cancellate - tag di corso'; // ORPHANED
$string['entertags'] = 'Inserite i tag separati da virgole'; // ORPHANED
$string['errordeleting'] = 'Errore in cancellazione tag con id $a, avvertire l\'amministratore di sistema.'; // ORPHANED
$string['errortagfrontpage'] = 'Il tagging non è consentito nella pagina home'; // ORPHANED
$string['errorupdatingrecord'] = 'Errore in aggiornamento record tag'; // ORPHANED
$string['manageofficialtags'] = 'Gestisci tag ufficiali'; // ORPHANED
$string['othertags'] = 'Altri tag (inserite i tag separati da virgole)'; // ORPHANED
$string['rssdesc'] = 'Questo feed RSS è stato generato automaticamente da Moodle e contiene tag create dall\'utente per i corsi.'; // ORPHANED
$string['rsstitle'] = 'Feed RSS per Tag di corso per l\'utente: $a'; // ORPHANED
$string['taggedwith'] = 'con il tag \"$a\"'; // ORPHANED
$string['thingtaggedwith'] = '\"$a->name\" usata una volta'; // ORPHANED

?>
