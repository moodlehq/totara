<?php
// access.php - created with Totara langimport script version 1.1

$string['access'] = 'Esteettömyys';
$string['accesshelp'] = 'Esteettömyysohje';
$string['accesskey'] = 'Näppäinkomento, $a';
$string['accessstatement'] = 'Esteettömyyslausunto';
$string['activitynext'] = 'Seuraava aktiviteetti';
$string['activityprev'] = 'Edellinen aktiviteetti';
$string['breadcrumb'] = 'Kulkujäljet';
$string['currenttopic'] = 'Tämä aihe';
$string['currentweek'] = 'Tämä viikko';
$string['hideblocka'] = 'Piilota $a lohko';
$string['monthnext'] = 'Seuraava kuukausi';
$string['monthprev'] = 'Edellinen kuukausi';
$string['showblocka'] = 'Näytä $a lohko';
$string['showhideblock'] = 'Näytä tai piilota lohko';
$string['sitemap'] = 'Sivukartta';
$string['skipa'] = 'Ohita';
$string['skipblock'] = 'Ohita lohko';
$string['skipnavigation'] = 'Ohita navigointivalikko';
$string['tabledata'] = 'Taulukko, $a';
$string['tablelayout'] = 'Asettelutaulukko, $a';
$string['tocontent'] = 'Pääsisältö';
$string['tonavigation'] = 'Navigointivalikkoon';
$string['youarehere'] = 'Olet tässä';

?>
