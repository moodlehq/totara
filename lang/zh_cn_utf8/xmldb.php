...
在生成之后，您就可以将这些语句复制到您喜爱的 SQL 界面中安全执行（不要忘记在这样做之前备份数据）。<br /><br />
强烈建议使用最新版本的 Moodle（1.8、1.9、2.x），之后再执行错误整数的搜索。<br /><br />
此功能不会对数据库执行任何操作（仅仅是读取数据库），因此您可以随时安全执行此功能。';
...
$string['nowrongdefaultsfound'] = '没有发现不一致的默认值，您的数据库不需要做任何更改。';
