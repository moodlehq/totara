...
class block_base {
    ...
    /**
     * A string generated by {@link _add_dashlet_edit_controls()} to display dashlet block manipulation links when the user is in editing mode.
     * @var string $dashlet_edit_controls
     */
    var $dashlet_edit_controls = NULL;
...
/// Class Functions
  ...
  // Intentionally doesn't check if a content_type is set. This is already done in _self_test()
    ...
    function _add_dashlet_block_edit_controls($options) {
        ...
        if ($options & BLOCK_CONFIGURE && $this->user_can_edit()) {
            $buttons .= '<a class="icon edit" title="'. $this->str->configure .'" href="'.$script.'&amp;blockaction=config">' .
                            '<img src="'. $CFG->pixpath .'/t/edit.gif" alt="'. $this->str->configure .'" /></a>';
        }

        $this->edit_controls = !empty($buttons) ? '<div class="commands">'.$buttons.'</div>' : '';
    }

    /**
     * Add dashlet-specific edit controls
     *
     * @uses $CFG
     * @uses $USER
     ...
     */
    function _add_dashlet_edit_controls($id, $controls, $options=array()) {
        global $CFG, $USER, $PAGE;
        ...
        $page = $PAGE;
        $queryparams = array('dlet' => $id, 'sesskey' => $USER->sesskey);
        if (!empty($options)) {
            $queryparams = array_merge($queryparams, $options);
        }
        $script = $page->url_get_full($queryparams);

        $movebuttons .= '<a class="icon hide" title="'. $title .'" href="'.$script.'&amp;dashaction=toggle">' .
                        '<img src="'. $CFG->pixpath.$icon .'" alt="'.$title.'" /></a>';

        if (!empty($controls['configure'])) {
            $movebuttons .= '<a class="icon edit" title="'. $this->str->configure .'" href="'.$script.'&amp;dashaction=config">' .
                            '<img src="'. $CFG->pixpath .'/t/edit.gif" alt="'. $this->str->configure .'" /></a>';
        }

        $movebuttons .= '<a class="icon delete" title="'. $this->str->delete .'" href="'.$script.'&amp;dashaction=delete">' .
                        '<img src="'. $CFG->pixpath .'/t/delete.gif" alt="'. $this->str->delete .'" /></a>';

        if (!empty($controls['moveleft'])) {
            $movebuttons .= '<a class="icon left" title="'. $this->str->moveleft .'" href="'.$script.'&amp;dashaction=moveleft">' .
                            '<img src="'. $CFG->pixpath .'/t/'.$leftarrow.'" alt="'. $this->str->moveleft .'" /></a>';
        }
        if (!empty($controls['moveup'])) {
            $movebuttons .= '<a class="icon up" title="'. $this->str->moveup .'" href="'.$script.'&amp;dashaction=moveup">' .
                            '<img src="'. $CFG->pixpath .'/t/up.gif" alt="'. $this->str->moveup .'" /></a>';
        }
        if (!empty($controls['movedown'])) {
            $movebuttons .= '<a class="icon down" title="'. $this->str->movedown .'" href="'.$script.'&amp;dashaction=movedown">' .
                            '<img src="'. $CFG->pixpath .'/t/down.gif" alt="'. $this->str->movedown .'" /></a>';
        }
        if (!empty($controls['moveright'])) {
            $movebuttons .= '<a class="icon right" title="'. $this->str->moveright .'" href="'.$script.'&amp;dashaction=moveright">' .
                            '<img src="'. $CFG->pixpath .'/t/'.$rightarrow.'" alt="'. $this->str->moveright .'" /></a>';
        }

        $movebuttons .= '</div>';

        $this->dashlet_edit_controls = $movebuttons;
    }
    ...
    function _print_dashlet_shadow() {
        print_side_block($this->_dashlet_title_html(), '&nbsp;', NULL, NULL, $this->dashlet_edit_controls, array('class' => 'dashlet'), $this->title);
    }
