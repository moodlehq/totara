 ...
 * @return boolean
 */
function priority_scale_is_used($scaleid) {
    global $CFG;

    // Inner join the framework table to ignore any
    // old priority assignments from when deleting a
...
function priorities_available() {
    ...
    $sql = "
        SELECT
            id,
            name
        FROM {$CFG->prefix}idp_tmpl_priority_scale scale
        WHERE EXISTS
        (
            SELECT
                1
            FROM
                {$CFG->prefix}idp_tmpl_scale_values scaleval
            WHERE
                scaleval.priorityscaleid = scale.id
        )
        ORDER BY
            name ASC
    ";
 ...
 * @return html
 */
function priority_display_table($priorities, $editingon=0) {
    global $CFG;
    ...
    if ($priorities) {
        ...
        foreach($priorities as $priority) {
            $line = array();
            $line[] = "<a href=\"$CFG->wwwroot/idp/priority/view.php?id={$priority->id}\">".format_string($priority->name)."</a>";
            ...
            if ($editingon) {
                if ($can_edit) {
                    $buttons[] = "<a title=\"$stredit\" href=\"$CFG->wwwroot/idp/priority/edit.php?id=$priority->id\"><img".
                        " src=\"$CFG->pixpath/t/edit.gif\" class=\"iconsmall\" alt=\"$stredit\" /></a> ";
                }

                if ($can_delete) {
                    $buttons[] = "<a title=\"$strdelete\" href=\"$CFG->wwwroot/idp/priority/delete.php?id=$priority->id\"><img".
    ...
    print_heading(get_string('priorityscales', 'idp'));

    if ($priorities) {
        print_table($table);
    } else {
        echo '<p>'.get_string('noprioritiesdefined', 'idp').'</p>';
    }

    echo '<div class="buttons">';
    print_single_button("$CFG->wwwroot/idp/priority/edit.php", null, get_string('prioritiescustomcreate', 'idp'));
    echo '</div>';
}
?>
