...
function dp_display_manager_overview($planuser) {
    global $CFG, $USER;
    $user = get_record('user', 'id', $planuser);
    if(!$user) {
        return false;
    ...
    $out = '<div class="plan_manager_overview">';
    ...
    // Add the header
    $out .= '<h2 class="main">' . get_string('userslearning', 'local_plan', $a) . '</h2>';

    // Add the caption
    $out .= '<p>' . get_string('managingyourteam', 'local_plan') . '</p>';

    // Add the users profile picture
    $out .= '<div class="plan_box_image">' . print_user_picture($user, 1, null, 0, true) . '</div>';

    // Calculate the number of unapproved learning plans and unapproved components
    $plan_records = dp_get_plans($planuser,array(DP_PLAN_STATUS_UNAPPROVED,DP_PLAN_STATUS_APPROVED));
    $action_strings = array(); //Store the action strings here
    if (!empty($plan_records)) {
	foreach ($plan_records as $plan_record) {
	    if ($plan_record->status == DP_PLAN_STATUS_UNAPPROVED) {
		$a = new object();
		$a->planname = $plan_record->name;
		$action_strings[] = '<a href="'.$CFG->wwwroot.'/local/plan/view.php?id='.$plan_record->id.'">' . get_string('planawaitingapproval', 'local_plan', $a) . '</a>';
	    }
	}
	...
	$plans = array();
	foreach ($plan_records as $plan_record) {
	    $plans[] = new development_plan($plan_record->id);
	}

	// TODO: Get this list of components from somewhere?
	// Only get theactive components in the site?
	$components = array('competency','course','objective');
	foreach ($components as $component) {
	    require_once($CFG->dirroot . "/local/plan/components/{$component}/{$component}.class.php");
	}

	// For each plan, calculate the number of unapproved items
	foreach ($plans as $plan) {
	    $num_unapproved_components = 0;
	    foreach ($components as $component) {
		$component_class = "dp_{$component}_component";
		$component_instance = new $component_class($plan);
		$component_instances = $component_instance->get_assigned_items(DP_APPROVAL_UNAPPROVED);
		$num_unapproved_components += count($component_instances);
	    }

	    $a = new object();
	    $a->planname = $plan->name;
	    if ($num_unapproved_components > 1) {
		$a->amount = $num_unapproved_components;
		$action_strings[] = '<a href="'.$CFG->wwwroot.'/local/plan/approve.php?id='.$plan->id.'">' . get_string('itemsawaitingapproval', 'local_plan', $a) . '</a>';
	    }
	    else if ($num_unapproved_components == 1) {
		$action_strings[] = '<a href="'.$CFG->wwwroot.'/local/plan/approve.php?id='.$plan->id.'">' . get_string('itemawaitingapproval', 'local_plan', $a) . '</a>';
	    }
	}
    ...
    if (count($action_strings) > 0) {
	$out .= '<div class="plan_box_actions_needed">';
	$out .= '<h3>'.get_string('actionneeded', 'local_plan').':</h3>';
	foreach ($action_strings as $action_string) {
	    $out .= $action_string;
	}
	$out .= '</div>'; //close plan_box_actions_needed
    }

    // Add the options available to the manager, such as sending a message or viewing the profile
    $out .= '<div class="plan_box_options">';
    $a = new object();
    $a->firstname = $user->firstname;
    if (!$user->deleted && !empty($CFG->messaging) && !isguest() && $USER->id != $user->id && has_capability('moodle/site:sendmessage', get_context_instance(CONTEXT_SYSTEM))) {
	    $url = "return openpopup('/message/discussion.php?id={$user->id}', 'message_{$user->id}', 'menubar=0,location=0,scrollbars,status,resizable,width=400,height=500', 0);";
	    $out .= '<a class="option_message" href="#" onclick="'.$url.'" >'.get_string('sendmessage','local_plan',$a).'</a>';
    }
    $out .= '<a class="option_profile" href="'.$CFG->wwwroot.'/user/view.php?id='.$user->id.'&course=1">'.get_string('viewprofile','local_plan',$a).'</a>';
    //$out .= '<a class="option_evaluation" href="#">'.get_string('startevaluation','local_plan').'</a>';
    $out .= '</div>';
