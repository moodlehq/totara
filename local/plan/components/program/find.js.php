...
totaraDialog_handler_preRequisite.prototype._update = function(response) {
    ...
    var noitems = $(response).filter('span.noitems-assignprograms');
    ...
    <?php
        ...
        if ($plan->get_component('program')->can_update_settings(false)) {
    ?>

    <?php if ($plan->get_component('program')->get_setting('duedatemode') && $plan->get_component('program')->get_setting('setduedate') >= DP_PERMISSION_ALLOW) { ?>
        // Add duedate datepicker
        $(function() {
            $('[id^=duedate_program]').datepicker(
                {
                    dateFormat: 'dd/mm/y',
                    showOn: 'both',
