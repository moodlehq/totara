...
print "Loading data for table 'competency_template'<br>";
...
foreach($items as $item) {
    if(get_field('competency_template', 'id', 'id', $item['id'])) {
        print "Record with id of {$item['id']} already exists!<br>\n";
        continue;
    }
    $newid = insert_record('competency_template',(object) $item);
    if($newid != $item['id']) {
        if(!set_field('competency_template', 'id', $item['id'], 'id', $newid)) {
            print "Could not change id from $newid to {$item['id']}<br>\n";
            continue;
        }
    }
    // record the highest id in the table
    $maxid = get_field_sql('SELECT '.sql_max('id').' FROM '.$CFG->prefix.'competency_template');
    // make sure sequence is higher than highest ID
    bump_sequence('competency_template', $CFG->prefix, $maxid);
    // print output
