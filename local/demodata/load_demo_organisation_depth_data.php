...
print "Loading data for table 'organisation_depth'<br>";
...
foreach($items as $item) {
    if(get_field('organisation_depth', 'id', 'id', $item['id'])) {
        print "Record with id of {$item['id']} already exists!<br>\n";
        continue;
    }
    $newid = insert_record('organisation_depth',(object) $item);
    if($newid != $item['id']) {
        if(!set_field('organisation_depth', 'id', $item['id'], 'id', $newid)) {
            print "Could not change id from $newid to {$item['id']}<br>\n";
            continue;
        }
    }
    // record the highest id in the table
    $maxid = get_field_sql('SELECT '.sql_max('id').' FROM '.$CFG->prefix.'organisation_depth');
    // make sure sequence is higher than highest ID
    bump_sequence('organisation_depth', $CFG->prefix, $maxid);
    // print output
