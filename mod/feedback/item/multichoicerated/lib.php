<?php  // $Id: lib.php,v 1.1.2.5 2010/09/03 20:15:22 agrabs Exp $
...
class feedback_item_multichoicerated extends feedback_item_base {
        ...
        ?>
                <td valign="top" align="<?php echo $align;?>"><input type="radio"
                        ...
                        value="<?php echo $index;?>" <?php echo $checked;?> />
                                ...
                                if($edit) {
                                    echo format_text('('.$radio_value[0].') '.$radio_value[1], true, false, false);
                                }else {
    ...
    function print_item_dropdown($item, $value, $info, $align, $edit, $lines) {
        ...
        foreach($lines as $line){
            ...
            if($edit) {
                echo '<option value="'. $index.'" '. $selected.'>'. clean_text('('.format_text($dropdown_value[0]).') '.$dropdown_value[1]).'</option>';
            }else {
                echo '<option value="'. $index.'" '. $selected.'>'. clean_text(format_text($dropdown_value[1])).'</option>';
            }
            $index++;
        }
        echo '</select>';
