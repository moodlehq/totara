    ...
    function excelprint_item(&$worksheet, $rowOffset, $item, $groupid, $courseid = false, $colOffset = 0, $facetofacesessionid = false) {
        ...
        if(is_array($data)) {
            for($i = 0; $i < sizeof($data); $i++) {
                ...
                $worksheet->write_string($rowOffset, $i + $colOffset + 1, trim($aData->answertext));
             
                $worksheet->setFormat("<l><vo>");
                $worksheet->write_number($rowOffset + 1, $i + $colOffset + 1, $aData->answercount);
                $worksheet->setFormat("<l><f><vo><pr>");
                $worksheet->write_number($rowOffset + 2, $i + $colOffset + 1, $aData->quotient);
            }
        }
        $rowOffset +=3 ;
        return $rowOffset;
