...
class feedback_item_dropdown extends feedback_item_base {
   ...
   if(!$values && !$facetofacesessionid) return null;
    ...
    function print_analysed($item, $itemnr = 0, $groupid = false, $courseid = false) {
        ...
        if($analysedItem) {
            ...
            foreach($analysedVals as $val) {
                ...
                echo '<tr><td valign="top" align="left">-&nbsp;&nbsp;' . trim($val->answertext) . ':</td><td align="left" width="'.FEEDBACK_MAX_PIX_LENGTH.'"><img style=" vertical-align: baseline;" src="'.$pix.'" height="5" width="'.$pixwidth.'" />&nbsp;' . $val->answercount . (($val->quotient > 0)?'&nbsp;('. $quotient . '&nbsp;%)':'').'</td></tr>';
            }
            //echo '</table>';
        }
        return $itemnr;
    ...
    function excelprint_item(&$worksheet, $rowOffset, $item, $groupid, $courseid = false, $colOffset = 0, $facetofacesessionid = false) {
        ...
        //frage schreiben
        $worksheet->write_string($rowOffset, $colOffset, stripslashes($analysed_item[1]));
        if(is_array($data)) {
