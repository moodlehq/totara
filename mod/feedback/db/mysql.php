...
function feedback_upgrade($oldversion) {
   ...
   if ($oldversion < 2005100307) {
      execute_sql(" ALTER TABLE `{$CFG->prefix}feedback` ADD `publish_stats` int(1) NOT NULL DEFAULT '0' AFTER `multiple_submit` ");
   }
   ...
   if ($oldversion < 2006070300) {
      execute_sql( " CREATE TABLE `{$CFG->prefix}feedback_sitecourse_map` (
               `id` int(10) unsigned NOT NULL auto_increment,
   ...
   if ($oldversion < 2006091100) {
      execute_sql(" ALTER TABLE `{$CFG->prefix}feedback` ADD `page_after_submit` TEXT NOT NULL AFTER `multiple_submit` ");
      execute_sql(" ALTER TABLE `{$CFG->prefix}feedback` ADD `timeopen` int(10) NOT NULL DEFAULT '0' AFTER `publish_stats` ");
      execute_sql(" ALTER TABLE `{$CFG->prefix}feedback` ADD `timeclose` int(10) NOT NULL DEFAULT '0' AFTER `timeopen` ");
   }
