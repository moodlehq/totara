...
//===== 1.9.0 upgrade line ======//
    ...
    // TOTARA2 SPECIAL UPGRADE
    // replay a lot of mod/scorm upgrades that have been lost to Totara due to
    // version issues
    if ($oldversion < 2011110503) {
        /// START OF SPECIAL UPGRADE REPLAYS FOR TOTARA2
        ...
        // set allow type defaults
        $allowtypeexternal = get_config('scorm', 'allowtypeexternal');
        if (empty($allowtypeexternal)) {
            $count = $DB->count_records('scorm', array('scormtype'=>'external'));
            if (!empty($count)) {
                set_config('allowtypeexternal', '1', 'scorm');
            }
        }
        $allowtypelocalsync = get_config('scorm', 'allowtypelocalsync');
        if (empty($allowtypelocalsync)) {
            $count = $DB->count_records('scorm', array('scormtype'=>'localsync'));
            if (!empty($count)) {
                set_config('allowtypelocalsync', '1', 'scorm');
            }
        }
        $allowtypeimsrepository = get_config('scorm', 'allowtypeimsrepository');
        if (empty($allowtypeimsrepository)) {
            $count = $DB->count_records('scorm', array('scormtype'=>'imsrepository'));
            if (!empty($count)) {
                set_config('allowtypeimsrepository', '1', 'scorm');
            }
        ...
        upgrade_mod_savepoint(true, 2011110503, 'scorm');
        // END OF SPECIAL UPGRADE REPLAYS FOR TOTARA2
    }
