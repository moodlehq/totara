<?php
    ...
    $can_add = has_capability('moodle/local:create'.$hierarchy->prefix.'frameworks', $sitecontext);
    $can_edit = has_capability('moodle/local:update'.$hierarchy->prefix.'frameworks', $sitecontext);
    $can_delete = has_capability('moodle/local:delete'.$hierarchy->prefix.'frameworks', $sitecontext);
...
if ($frameworks) {
    ...
    $table->head = array();
    $table->align = array();
    $table->head[] = get_string('framework', $hierarchy->prefix);
    $table->align[] = 'left';
    ...
    foreach ($frameworks as $framework) {
        ...
        $row[] = "<a $cssclass href=\"{$CFG->wwwroot}/{$hierarchy->prefix}/index.php?frameworkid={$framework->id}\">{$framework->fullname}</a>";

        // Add edit link
        $buttons = array();
        if ($editingon && $can_edit) {
            $buttons[] = "<a href=\"{$CFG->wwwroot}/{$hierarchy->prefix}/frameworks/edit.php?id={$framework->id}\" title=\"$str_edit\">".
                "<img src=\"{$CFG->pixpath}/t/edit.gif\" class=\"iconsmall\" alt=\"$str_edit\" /></a>";
            if ($framework->visible) {
                $buttons[] = "<a href=\"{$CFG->wwwroot}/{$hierarchy->prefix}/frameworks/index.php?hide={$framework->id}\" title=\"$str_hide\">".
                    "<img src=\"{$CFG->pixpath}/t/hide.gif\" class=\"iconsmall\" alt=\"$str_hide\" /></a>";
            } else {
                $buttons[] = "<a href=\"{$CFG->wwwroot}/{$hierarchy->prefix}/frameworks/index.php?show={$framework->id}\" title=\"$str_show\">".
                    "<img src=\"{$CFG->pixpath}/t/show.gif\" class=\"iconsmall\" alt=\"$str_show\" /></a>";
            }
        }
        if ($editingon && $can_delete) {
            $buttons[] = "<a href=\"{$CFG->wwwroot}/{$hierarchy->prefix}/frameworks/delete.php?id={$framework->id}\" title=\"$str_delete\">".
...
if ($frameworks) {
    ...
    print_heading(get_string('noframeworks', $hierarchy->prefix));
}
...
if ($can_add) {
    ...
    print_single_button($CFG->wwwroot.'/'.$hierarchy->prefix.'/frameworks/edit.php', array(), get_string('addnewframework', $hierarchy->prefix), 'get');
