...
class fusion_grade_export_form extends moodleform {
...
//            $mform->setDefault('separator', 'comma');
        ...
        if ($grade_items = $gseq->items) {
            ...
            foreach ($grade_items as $grade_item) {
                $cnt++;
                // crudely find the total line - how else do we do this?
                if (preg_match('/total/', $grade_item->get_name()) && count($grade_items)) {
                    $flag = 0;
                }
                else {
                    $flag = 1;
                ...
                if (!empty($features['idnumberrequired']) and empty($grade_item->idnumber)) {
                    ...
                    $mform->setDefault('itemids['.$grade_item->id.']', $flag);
