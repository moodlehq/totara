...
class profile_define_datetime extends profile_define_base {
    ...
    function define_form_specific($form) {
        ...
        // Add elements
        $form->addElement('select', 'param1', get_string('startyear', 'admin'), $arryears);
        $form->setType('param1', PARAM_INT);
        $form->setDefault('param1', $currentyear);

        $form->addElement('select', 'param2', get_string('endyear', 'admin'), $arryears);
        $form->setType('param2', PARAM_INT);
        $form->setDefault('param2', $currentyear + 20);

        $form->addElement('checkbox', 'param3', get_string('wanttime', 'admin'));
    ...
    function define_validate_specific($data) {
        ...
        if ($data->param1 > $data->param2) {
            $errors['param1'] = get_string('startyearafterend', 'admin');
        }
