...
M.util.help_icon = {
    ...
    add : function(Y, properties) {
        ...
        if (properties.node) {
            ...
            Y.use('event-hover', function(Y) {
                var on = M.util.image_url('help-hover', 'moodle'),
                    off = M.util.image_url('help', 'moodle');
                properties.node.on('hover',
                    function(e){
                        this.one('img.iconhelp').set('src', on);
                    },
                    function(e){
                        this.one('img.iconhelp').set('src', off);
                    });
            });
        }
    },
        ...
        if (M.util.help_icon.instance === null) {
            ...
            Y.use('overlay', 'io-base', 'event-mouseenter', 'node', 'event-key', function(Y) {
                var help_content_overlay = {
                    ...
                    display : function(event, args) {
                        var isRTL = (M.util.get_string('thisdirection', 'langconfig') === 'rtl');
                        var overlaypos = (isRTL) ? Y.WidgetPositionAlign.TR : Y.WidgetPositionAlign.TL;
                        var nodepos = (isRTL) ? Y.WidgetPositionAlign.LC : Y.WidgetPositionAlign.RC;
                        this.helplink = args.node;

                        this.overlay.set('bodyContent', Y.Node.create('<img src="'+M.cfg.loadingicon+'" class="spinner" />'));
                        this.overlay.set("align", {node:args.node, points:[overlaypos, nodepos]});
