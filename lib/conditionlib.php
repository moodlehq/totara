...
class condition_info {
    ...
    public function __construct($cm, $expectingmissing=CONDITION_MISSING_NOTHING,
        ...
        if (!isset($cm->availablefrom) || !isset($cm->availableuntil) || 
            ...
            $cm = get_record('course_modules', 'id', $cm->id, null, null, null, null, 'id,course,availablefrom,availableuntil,showavailability');
        }
...
FROM
    {$CFG->prefix}course_modules_availability cma
    LEFT JOIN {$CFG->prefix}grade_items gi ON gi.id=cma.gradeitemid
WHERE
    ...
    public function add_completion_condition($cmid, $requiredcompletion) {
        // Add to DB
        insert_record('course_modules_availability',
    ...
    public function wipe_conditions() {
        // Wipe from DB
        delete_records('course_modules_availability', 'coursemoduleid', $this->cm->id);
    ...
    public function get_full_information($modinfo=null) {
        ...
        if(count($this->cm->conditionscompletion)>0) {
            if ($this->cm->course==$COURSE->id) {
                ...
                $course = get_record('course', 'id', $this->cm->course, null, null, null, null, 'id,enablecompletion,modinfo');
            }
    ...
    public function is_available(&$information, $grabthelot=false, $userid=0, $modinfo=null) {
        ...
        if(count($this->cm->conditionscompletion)>0) {
            ...
            foreach ($this->cm->conditionscompletion as $cmid=>$expectedcompletion) {
                ...
                if (empty($modinfo->cms[$cmid])) {
                    // Activity has condition on deleted activity
                    continue;
                }
...
FROM 
    ...
    LEFT JOIN {$CFG->prefix}grade_grades gg ON gi.id=gg.itemid AND gg.userid = {$USER->id}
WHERE
    gi.courseid = {$this->cm->course}");
                    ...
                    $record = get_record('grade_grades', 'userid', $USER->id, 'itemid', $gradeitemid);
            ...
            $record = get_record('grade_grades', 'userid', $userid, 'itemid', $gradeitemid);
            if ($record && !is_null($record->finalgrade)) {
                $score = (($record->finalgrade - $record->rawgrademin) * 100) /
                    ($record->rawgrademax - $record->rawgrademin);
            } else {
